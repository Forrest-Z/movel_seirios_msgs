      ...
      if (current_index_ <= loaded_path_.poses.size() - 1 && skip_on_obstruction_) 
      {
        ...
        else
        {
          ...
          if (current_index_ >= loaded_path_.poses.size()-1)
          {
            ...
            std_msgs::Bool boolean;
            boolean.data = false;
            ROS_INFO("[%s] completion, final waypoint obstructed, skip is true", name_.c_str());
            start_pub_.publish(boolean);
            return;
          }
...
void PathLoadSegments::getPose(const geometry_msgs::Pose::ConstPtr &msg) 
{
  ...
  if (obstructed_) {
             ...
             && clearing_timeout_ > 0) 
    {
      ...
      ROS_INFO("[%s] Waiting for obstacle clearance exceeded timeout (no viable path), skipping waypoint to %lu", 
                name_.c_str(), current_index_);
      obstructed_ = false;
      waiting_for_obstacle_clearance_ = false;
      Resume();
      return;
...
void PathLoadSegments::onGoal(const move_base_msgs::MoveBaseActionResult::ConstPtr &msg) 
{
  ...
  if (end_ && (msg->status.status == 3 || msg->status.status == 4)) {
    ...
    ROS_INFO("[%s] completion check idx %lu, dxy %5.2f/%5.2f, dyaw %5.2f/%5.2f",
             name_.c_str(), current_index_, dxy, mb_xy_tolerance_, dyaw, mb_yaw_tolerance_);
    ...
    else
    {
      ...
      ROS_INFO("[%s] 2.linear %5.2f out of %5.2f", name_.c_str(), dxy, mb_xy_tolerance_);
      ROS_INFO("[%s] 2.angular %5.2f of %5.2f", name_.c_str(), dyaw, mb_yaw_tolerance_);
      publishPath(loaded_path_.poses[current_index_].pose, true);
    }
  }
}
