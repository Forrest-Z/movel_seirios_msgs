/*!
 *  This is a path loading code that loads paths from yaml files, and executes the path by sending waypoints of the path
 * in sequence to move_base node.
 */
...
private:
  ...
  nav_msgs::Path loaded_path;        //!< Path loaded for execution

  void publishPath();                           //!< Publish waypoints of path
  void populateClient(nav_msgs::GetPlan& srv);  //!< Populate service client for calling move_base/make_plan service
  double calculateLength(nav_msgs::Path plan);  //!< Calculate length of path plan
...
public:
  ...
  double update_time_interval;  //!< Time interval for publishing waypoints
  ...
  bool onPause(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res);  //!< ROS callback

  //! Resume loading
  bool Resume();                                                                     //!< Resume path following
  bool onResume(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res);  //!< ROS callback

  //! Cancel loading
  ros::Publisher cancel_pub_;  //!< Publisher to topic for cancelling goal
  void Cancel();               //!< Cancel path following
  bool onCancel(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res);  //!< ROS callback

  //! Check current pose of robot
  bool Check(std::string name, float threshold);  //!< Check robot pose w.r.t. first waypoint
  bool onCheck(path_recall::PathCheck::Request& req, path_recall::PathCheck::Response& res);  //!< ROS callback
};
