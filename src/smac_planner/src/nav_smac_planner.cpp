...
SmacPlanner::~SmacPlanner()
{
  ROS_INFO_STREAM("[smac_planner] Destroying plugin %s of type SmacPlanner" << _name.c_str());
  if (dsrv_)
    delete dsrv_;
}
...
void SmacPlanner::initialize(std::string name, costmap_2d::Costmap2DROS * costmap_ros)
{
  ...
  dynamic_reconfigure::Server<smac_planner::SmacPlannerConfig>::CallbackType cb = boost::bind(&SmacPlanner::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);

  make_plan_srv_ = node_handle.advertiseService("make_plan", &SmacPlanner::makePlanService, this);
  _plan_publisher =
...
{
  ROS_INFO_STREAM("[smac_planner] Activating plugin " << _name.c_str() << " of type SmacPlanner");
...
{
  ROS_INFO_STREAM("[smac_planner] Deactivating plugin " << _name.c_str() << " of type SmacPlanner");
  if (_costmap_downsampler) {
...
{
  ROS_INFO_STREAM("[smac_planner] Cleaning up plugin " << _name.c_str() << " of type SmacPlanner");
  _a_star.reset();
  _smoother.reset();
...
bool SmacPlanner::makePlanService(nav_msgs::GetPlan::Request& req, nav_msgs::GetPlan::Response& resp)
{
