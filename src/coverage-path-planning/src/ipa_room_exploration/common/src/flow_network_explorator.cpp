...
// possible nodes.
void FlowNetworkExplorator::solveThreeStageOptimizationProblem(
    std::vector<double>& C, const cv::Mat& V, const std::vector<double>& weights,
    ...
    const std::vector<uint>& start_arcs)
{
  ...
  for (size_t aux_flow = 0; aux_flow < V.cols + start_arcs.size();
       ++aux_flow)  // auxiliary flow variables for initial and coverage stage
  {
  ...
  for (size_t indicator = 0; indicator < flows_into_nodes.size();
       ++indicator)  // indicator variables showing if a node is in the path
  {
    problem_builder.setColBounds(number_of_variables, 0.0, 1.0);
    problem_builder.setObjective(number_of_variables, 0.0);  // no additional part in the objective
    problem_builder.setInteger(number_of_variables);
  ...
  for (size_t node = 0; node < flows_into_nodes.size(); ++node)
  {
    ...
    // vectors for the decreasing equality constraint that ensures that the flow gets reduced by 1, every time it passes
    // a node,
    // cycles are prevented by this, because a start of a cycle is also an end of it, producing a constraint that the
    // flow
    ...
    for (size_t inflow = 0; inflow < flows_into_nodes[node].size(); ++inflow)
    {
      ...
      {
        // conservativity
        variable_indices.push_back(std::find(start_arcs.begin(), start_arcs.end(), flows_into_nodes[node][inflow]) -
                                   start_arcs.begin());
    ...
    problem_builder.addRow((int)flow_decrease_indices.size(), &flow_decrease_indices[0], &flow_decrease_coefficients[0],
                           0.0, 0.0);
...
// Function that creates a Gurobi optimization problem and solves it, using the given matrices and vectors and the
// 3-stage
// ansatz, that takes an initial step going from the start node and then a coverage stage assuming that the number of
// flows into and out of a node must be the same. At last a final stage is gone, that terminates the path in one of the
...
void FlowNetworkExplorator::solveGurobiOptimizationProblem(std::vector<double>& C, const cv::Mat& V,
                                                           const std::vector<double>& weights,
                                                           const std::vector<std::vector<uint> >& flows_into_nodes,
                                                           const std::vector<std::vector<uint> >& flows_out_of_nodes,
                                                           const std::vector<uint>& start_arcs)
{
#ifdef GUROBI_FOUND
  ...
  CyclePreventionCallbackClass callback_object =
      CyclePreventionCallbackClass(&optimization_variables, V.cols, flows_out_of_nodes, flows_into_nodes, start_arcs);
...
// is detected in the solution or the only cycle contains all visited nodes, because such a solution is a traveling
// salesman like solution, which is a valid solution.
void FlowNetworkExplorator::solveLazyConstraintOptimizationProblem(
    std::vector<double>& C, const cv::Mat& V, const std::vector<double>& weights,
    const std::vector<std::vector<uint> >& flows_into_nodes, const std::vector<std::vector<uint> >& flows_out_of_nodes,
    const std::vector<uint>& start_arcs)
{
  ...
  do
  {
    ...
    std::set<uint> used_arcs;  // set that stores the indices of the arcs corresponding to non-zero elements in the
                               // solution
    ...
    int number_of_strong_components = boost::strong_components(
        support_graph,
        boost::make_iterator_property_map(c.begin(), boost::get(boost::vertex_index, support_graph), c[0]));
    ...
    if (cycle_free == false)
    {
      ...
      for (size_t cycle = 0; cycle < cycle_nodes.size(); ++cycle)
      {
        ...
        //							int flow_index = -1;
        //							for(std::vector<uint>::const_iterator
        //outflow=flows_out_of_nodes[cycle_nodes[cycle][neighbor]].begin();
        //outflow!=flows_out_of_nodes[cycle_nodes[cycle][neighbor]].end(); ++outflow)
        //							{
        ...
        //					// add the constraint
        //					solver_pointer->addRow((int) cpc_indices.size(), &cpc_indices[0], &cpc_coefficients[0], 0.0,
        //COIN_DBL_MAX);
        //				}
        ...
        for (size_t node = 0; node < cycle_nodes[cycle].size(); ++node)
        {
          for (std::vector<uint>::const_iterator outflow = flows_out_of_nodes[cycle_nodes[cycle][node]].begin();
               outflow != flows_out_of_nodes[cycle_nodes[cycle][node]].end(); ++outflow)
          {
            for (size_t neighbor = 0; neighbor < cycle_nodes[cycle].size(); ++neighbor)
            {
              if (neighbor == node)
              ...
              // if a cycle-node contains this arc is incoming node and was used in the solution, it belongs to the
              // subset
              if (contains(flows_into_nodes[cycle_nodes[cycle][neighbor]], *outflow) == true &&
                  used_arcs.find(*outflow) != used_arcs.end())
              {
                cpc_indices.push_back(*outflow + start_arcs.size());
                cpc_coefficients.push_back(1.0);
              }
        ...
        solver_pointer->addRow((int)cpc_indices.size(), &cpc_indices[0], &cpc_coefficients[0], COIN_DBL_MIN,
                               cycle_nodes[cycle].size() - 1);
      }
...
// This Function checks if the given cv::Point is close enough to one cv::Point in the given vector. If one point gets
// found
// that this Point is nearer than the defined min_distance the function returns false to stop it immediately.
bool FlowNetworkExplorator::pointClose(const std::vector<cv::Point>& points, const cv::Point& point,
                                       const double min_distance)
{
  double square_distance = min_distance * min_distance;
  for (std::vector<cv::Point>::const_iterator current_point = points.begin(); current_point != points.end();
       ++current_point)
  {
...
// direction is
//		found that suits best for calculating the edges, i.e. such that longer edges occur, and the map is rotated in this
//manner.
//		This allows to use the algorithm as it was and in the last step, the found path points simply will be transformed
//back to the
//		original orientation.
//	II.	Discretize the free space into cells that have to be visited a least once by using the sampling distance given
//to
...
void FlowNetworkExplorator::getExplorationPath(const cv::Mat& room_map, std::vector<geometry_msgs::Pose2D>& path,
                                               ...
                                               const cv::Point2d map_origin, const int cell_size,
                                               const Eigen::Matrix<float, 2, 1>& robot_to_fov_middlepoint_vector,
                                               const float coverage_radius, const bool plan_for_footprint,
                                               const double path_eps, const double curvature_factor,
                                               const double max_distance_factor)
{
  ...
  // todo: if first/last row or column in grid has accessible areas but center is inaccessible, create a node in the
  // accessible area
  ...
  for (size_t y = min_y - 1; y <= max_y + 1; ++y)
  {
    ...
    for (size_t x = min_x; x <= max_x + 1; x += 2.0 * coverage_int)
    {
      ...
      if (rotated_room_map.at<uchar>(y, x) == 0 &&
          (rotated_room_map.at<uchar>(y - 1, x) == 255 || rotated_room_map.at<uchar>(y + 1, x) == 255))
      {
  ...
  //		{
  //			// check if an obstacle has been found, only check outer parts of the occupied space
  //			if(rotated_room_map.at<uchar>(y,x)==0 && (rotated_room_map.at<uchar>(y,x-1)==255 ||
  //rotated_room_map.at<uchar>(y,x+1)==255))
  //			{
  ////				cv::circle(test_map, cv::Point(x,y), 2, cv::Scalar(127), cv::FILLED);
  //				// check on both sides along the sweep line if a free point is available, don't exceed matrix dimensions
  ...
  distance_matrix_computation.constructDistanceMatrix(distance_matrix, rotated_room_map, edges, 0.25, 0.0,
                                                      map_resolution, path_planner_);
  ...
  for (size_t start = 0; start < distance_matrix.rows; ++start)
  {
    for (size_t end = 0; end < distance_matrix.cols; ++end)
    {
      ...
      if (start != end && end > start)
      {
        ...
        if (current_forward_arc.weight <= max_distance_factor * max_distance &&
            current_forward_arc.weight <= curvature_factor * cv::norm(vector))
        {
          std::vector<cv::Point> astar_path;
          path_planner_.planPath(rotated_room_map, current_forward_arc.start_point, current_forward_arc.end_point, 1.0,
                                 0.0, map_resolution, 0, &astar_path);
  ...
  std::cout << "Constructed all matrices for the optimization problem. Checking if all cells can be covered."
            << std::endl;
  ...
  for (size_t row = 0; row < V.rows; ++row)
  {
    ...
    {
      std::cout << "!!!!!!!! EMPTY ROW OF VISIBILITY MATRIX !!!!!!!!!!!!!" << std::endl
                << "cell " << row << " not coverable" << std::endl;
      all_cells_covered = false;
    }
  }
  if (all_cells_covered == false)
    std::cout << "!!!!! WARNING: Not all cells could be covered with the given parameters, try changing them or ignore "
                 "it to not cover the whole free space."
              << std::endl;
  ...
  std::vector<double> C(2.0 * (flows_out_of_nodes[start_index].size() + number_of_candidates) + number_of_outflows +
                        edges.size());
...
#else
  solveLazyConstraintOptimizationProblem(C, V, w, flows_into_nodes, flows_out_of_nodes,
                                         flows_out_of_nodes[start_index]);
#endif
  ...
  // go trough the coverage stage
  for (size_t cover_arc = flows_out_of_nodes[start_index].size();
       cover_arc < flows_out_of_nodes[start_index].size() + arcs.size(); ++cover_arc)
  {
    //		cv::Mat test_map = rotated_room_map.clone();
    //		for(std::vector<cv::Point>::iterator p=edges.begin(); p!=edges.end(); ++p)
    //			cv::circle(test_map, *p, 2, cv::Scalar(100), cv::FILLED);
  ...
  uint path_end = 0;
  for (uint final_arc = flows_out_of_nodes[start_index].size() + arcs.size();
       final_arc < flows_out_of_nodes[start_index].size() + 2 * arcs.size(); ++final_arc)
  {
    //		cv::Mat test_map = rotated_room_map.clone();
    //		for(std::vector<cv::Point>::iterator p=edges.begin(); p!=edges.end(); ++p)
    //			cv::circle(test_map, *p, 2, cv::Scalar(100), cv::FILLED);
    if (C[final_arc] > 0.01)
    {
      ...
      //			std::vector<cv::Point>
      //path=arcs[final_arc-flows_out_of_nodes[start_index].size()-arcs.size()].edge_points;
      //			for(size_t j=0; j<path.size(); ++j)
  ...
  //		for(size_t flow=0; flow<flows_out_of_nodes[node].size(); ++flow)
  //		{
  //			if(C[flows_out_of_nodes[node][flow]+flows_out_of_nodes[start_index].size()+V.cols]>0.01) // taking integer
  //precision in solver into account
  //			{
  //				// insert saved outgoing flow index
  //				used_arcs.insert(flows_out_of_nodes[node][flow]);
  //				std::vector<cv::Point> path=arcs[flows_out_of_nodes[node][flow]].edge_points;
  ...
  do
  {
    ...
    for (std::set<uint>::iterator arc_index = used_arcs.begin(); arc_index != used_arcs.end(); ++arc_index)
    {
      ...
      // check if current arc has been gone already (e.g. when a cycle is appended to the rest of the path --> still a
      // valid solution)
...
void FlowNetworkExplorator::testFunc()
{
  ...
  cv::Mat V_reduced =
      cv::Mat(V.rows, 1, CV_8U);  // initialize one column because opencv wants it this way, add other columns later
