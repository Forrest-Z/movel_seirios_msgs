...
// Function that calculates an exploration path trough the given map s.t. everything has been covered by the
// robot-footprint
...
// I. 	Construct the neural network by sampling the given map, using the given fitting circle radius as step size. This
// is
//		done because it allows that when all neurons (the samples) are covered the whole space have been
//cleaned/inspected.
...
//		into a direction he is currently not pointing at. At every time-step an update of the states of the neurons is
//done.
...
void NeuralNetworkExplorator::getExplorationPath(const cv::Mat& room_map, std::vector<geometry_msgs::Pose2D>& path,
                                                 ...
                                                 bool show_path_computation)
{
  ...
  //			black_map.at<float>(row*fitting_radius_as_int, column*fitting_radius_as_int) = (float)
  //1000.0*neurons_[row][column].getState(false);
  //		}
  //		std::cout << std::endl;
  //	}
  //	std::cout << std::endl;
  ...
  double previous_traveling_angle =
      0.0;  // save the travel direction to the current neuron to determine the next neuron
  ...
  do
  {
    ...
    for (size_t neighbor = 0; neighbor < neighbors.size(); ++neighbor)
    {
      ...
      travel_angle = std::atan2(neighbors[neighbor]->getPosition().y - previous_neuron->getPosition().y,
                                neighbors[neighbor]->getPosition().x - previous_neuron->getPosition().x);
      ...
      // std::cout << " Neighbor: " << neighbors[neighbor]->getPosition() << "   " <<
      // neighbors[neighbor]->getState(false) << ", " << delta_theta_weight_ * y << std::endl;
    ...
    if (number_of_neuron_in_path >= 20)
    {
      ...
      for (std::vector<cv::Point>::iterator pose = fov_coverage_path.begin() + 1; pose != fov_coverage_path.end() - 1;
           ++pose)
      {
