...
//is transformed
//		s.t. this edge lies horizontal to the x-direction. This produces longer but fewer edges, what improves the path
//for small but long
//		cells. The startpoint of the cell-path is determined by the endpoint of the previous cell, s.t. the distance
//between two
//		cell-paths is minimized. The cell-path is determined in the rotated manner, so in a last step, the cell-path is
//transformed to
...
//robot
//		pose is not in the free space, another accessible point is generated by finding it on the radius around the field
//of view (fov)
                                               ...
                                               const double min_cell_area, const int max_deviation_from_track)
{
  ...
  cv::Point robot_starting_position =
      (fov_poses.size() > 0 ? cv::Point(cvRound(fov_poses[0].x), cvRound(fov_poses[0].y)) : starting_position);
  ...
  if (path.size() > 0)
    cv::circle(fov_path_map,
               cv::Point((path[0].x - map_origin.x) / map_resolution, (path[0].y - map_origin.y) / map_resolution), 2,
    ...
    else if (previous_number_of_segments > number_of_segments)  // OUT event
    {
      ...
      for (int x = 0; x < room_map.cols; ++x)
      {
        ...
        else if (hit_white_pixel == true && room_map.at<uchar>(y - 1, x) == 0)
        {
          ...
          bool critical_point = true;
          for (int dx = -1; dx <= 1; ++dx)
            if (room_map.at<uchar>(y, std::max(0, std::min(x + dx, room_map.cols - 1))) ==
                0)  // check at side after obstacle
              critical_point = false;
                                                     ...
                                                     const int max_deviation_from_track, const int grid_obstacle_offset)
{
  ...
  cv::invertAffineTransform(
      R_cell, R_cell_inv);  // invert the rotation matrix to remap the determined points to the original cell
