...
double EnergyFunctionalExplorator::E(const EnergyExploratorNode& location, const EnergyExploratorNode& neighbor,
                                     const double cell_size_in_pixel, const double previous_travel_angle)
{
  ...
  // std::cout << "E: " << cv::norm(diff)/cell_size << " + " << std::abs(diff_angle)*PI_2_INV << " + " << 4. -
  // 0.5*visited_neighbors << " + " << 0.72 - 0.09*wall_points << "                        angles: " <<
  // travel_angle_to_node << ", " << previous_travel_angle << "   diff ang: " << diff_angle << std::endl;
...
//
// This method discretizes the free space, that should be covered, into several nodes. Each of the node has to be
// covered, in order
// to cover the whole area. The path starts at the node that is closest to the given starting position and chooses the
// next node as
// the one that minimizes the energy functional, provided in the paper above. To do this here the following steps are
// done.
//	I.	The free area gets discretized into several nodes, using the given cell_size parameter, starting at the upper
//left white pixel of
//		the room. Whenever the overlaid grid then hits a white pixel, this point is added as a node. Then after all nodes
//have been found
...
//			i.	The start node gets chosen as the one that is closest to the given starting position and is an edge of the
//given room, i.e
//				a node that has less than 4 neighbors.
//			ii.	The next node is then iteratively chosen from the directly neighboring ones, by finding the node that
//minimizes the given
//				energy functional and wasn't visited before.
//			iii.If in the neighborhood no accessible point could be found, search for the next node in the whole grid to
//continue the path.
...
//		robot footprint poses. To do so simply a vector transformation is applied. If the computed robot pose is not in
//the
...
//
void EnergyFunctionalExplorator::getExplorationPath(const cv::Mat& room_map, std::vector<geometry_msgs::Pose2D>& path,
                                                    ...
                                                    const Eigen::Matrix<float, 2, 1> robot_to_fov_vector)
{
  ...
  for (int y = min_room.y + half_grid_spacing_as_int; y < max_room.y; y += grid_spacing_as_int)
  {
    ...
    for (int x = min_room.x + half_grid_spacing_as_int; x < max_room.x; x += grid_spacing_as_int)
    {
      ...
      // if(rotated_room_map.at<uchar>(y,x) == 255)				// could make sense to test all pixels of the cell, not only the
      // center
  ...
  std::vector<EnergyExploratorNode*> corner_nodes;  // vector that stores the corner nodes, i.e. nodes with 3 or less
                                                    // neighbors
  ...
  for (std::vector<EnergyExploratorNode*>::iterator corner = corner_nodes.begin(); corner != corner_nodes.end();
       ++corner)
  {
  ...
  double previous_travel_angle = 0;  // always use x-direction in the rotated map
                                     // //std::atan2(rotated_starting_point.y-start_node->center_.y,
                                     // rotated_starting_point.x-start_node->center_.x);
  ...
  {
    if ((*neighbor)->obstacle_ == false && (*neighbor)->center_.y == start_node->center_.y &&
  ...
  do
  {
    ...
    previous_travel_angle =
        std::atan2(next_node->center_.y - last_node->center_.y, next_node->center_.x - last_node->center_.x);
  ...
  //	// go trough the found fov-path and compute the angles of the poses s.t. it points to the next pose that should be
  //visited
  ...
  //	cv::Mat path_map = rotated_room_map.clone();
  //	cv::circle(path_map, fov_coverage_path[0], 2, cv::Scalar(100), cv::FILLED);
  //	for(std::vector<cv::Point>::iterator path_node=fov_coverage_path.begin()+1; path_node!=fov_coverage_path.end();
  //++path_node)
  //	{
  ...
  cv::Point robot_starting_position =
      (fov_poses.size() > 0 ? cv::Point(fov_poses[0].x, fov_poses[0].y) : starting_position);
