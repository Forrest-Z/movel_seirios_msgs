...
template <typename T>
void convexSPPExplorator::solveGurobiOptimizationProblem(std::vector<T>& C, const cv::Mat& V,
...
// Function that is used to get a coverage path that covers the free space of the given map. It is programmed according
// to
//
...
//
// In this paper a linear program is used to get the minimal set of sensing poses to check the whole area for gas, but
// it
...
//			used, that checks |{i: c[i] <= eps}|. In each step the weights are adapted with respect to the previous
//solution.
...
void convexSPPExplorator::getExplorationPath(const cv::Mat& room_map, std::vector<geometry_msgs::Pose2D>& path,
                                             ...
                                             const cv::Point2d map_origin, const int cell_size_pixel,
                                             const double delta_theta,
                                             const std::vector<Eigen::Matrix<float, 2, 1> >& fov_corners_meter,
                                             const Eigen::Matrix<float, 2, 1>& robot_to_fov_vector_meter,
                                             const double largest_robot_to_footprint_distance_meter,
                                             const uint sparsity_check_range, const bool plan_for_footprint)
{
  ...
  GridGenerator::generateBoustrophedonGrid(rotated_room_map, inflated_rotated_room_map, half_cell_size, grid_lines,
                                           cv::Vec4i(-1, -1, -1, -1), cell_size_pixel, half_cell_size, cell_size_pixel);
  ...
  for (std::vector<geometry_msgs::Pose2D>::iterator pose = candidate_sensing_poses.begin();
       pose != candidate_sensing_poses.end(); ++pose)
  {
    ...
    {
      pose_as_matrix << (pose->x * map_resolution) + map_origin.x,
          (pose->y * map_resolution) + map_origin.y;  // convert to [meter]
      for (size_t point = 0; point < fov_corners_meter.size(); ++point)
      {
        ...
        // values
        cv::Point current_point = cv::Point((transformed_vector(0, 0) - map_origin.x) * map_resolution_inverse,
                                            (transformed_vector(1, 0) - map_origin.y) * map_resolution_inverse);
    ...
    // when planning for the robot footprint simply check if its distance to the pose is at most the given coverage
    // radius
    for (std::vector<cv::Point>::iterator neighbor = cell_centers.begin(); neighbor != cell_centers.end(); ++neighbor)
    {
      ...
      if (plan_for_footprint == false && distance <= largest_robot_to_footprint_distance_pixel)
      {
        ...
        {
          // check if the line from the robot pose to the neighbor crosses an obstacle, if so it is not observable from
          // the pose
          cv::LineIterator border_line(room_map, cv::Point(pose->x, pose->y), *neighbor,
                                       8);  // opencv implementation of bresenham algorithm, 8: color, irrelevant
      ...
      else if (plan_for_footprint == true &&
               (distance + cell_outcircle_radius_pixel) <= largest_robot_to_footprint_distance_pixel)
      {
        // check if the line from the robot pose to the neighbor crosses an obstacle, if so it is not observable from
        // the pose
        cv::LineIterator border_line(room_map, cv::Point(pose->x, pose->y), *neighbor,
  ...
  //			if(V.at<uchar>(i, j) == 1)
  //			{
  //				cv::circle(black_map, cv::Point(candidate_sensing_poses[j].x, candidate_sensing_poses[j].y), 2,
...
// solve optimization of the current step
    ...
    if (sparsity_measures.size() >= sparsity_check_range)
    {
      ...
      for (std::vector<uint>::reverse_iterator measure = sparsity_measures.rbegin();
           measure != sparsity_measures.rbegin() + sparsity_check_range && measure != sparsity_measures.rend();
           ++measure)
    ...
    std::cout << "Iteration: " << number_of_iterations << ", sparsity: " << sparsity_measures.back() << std::endl;
           ...
           tim.getElapsedTimeInSec() < 1200);  // wait no longer than 20 minutes
  ...
  cv::Mat V_reduced = cv::Mat(cell_centers.size(), 1, CV_8U);  // initialize one column because opencv wants it this
                                                               // way, add other columns later
  ...
  for (std::vector<int>::iterator result = C_reduced.begin(); result != C_reduced.end(); ++result)
  {
    if (*result == 1)
    {
      ...
      chosen_positions.push_back(cv::Point(reduced_sensing_candidates[result - C_reduced.begin()].x,
                                           reduced_sensing_candidates[result - C_reduced.begin()].y));
    }
  }
  ...
  distance_matrix_computation.constructDistanceMatrix(distance_matrix, room_map, chosen_positions, 0.25, 0.0,
  ...
  for (size_t pose = 0; pose < chosen_poses.size(); ++pose)
  {
    for (size_t neighbor = 0; neighbor < chosen_poses.size(); ++neighbor)
    {
      ...
      if (pose != neighbor)
      {
        ...
        distance_matrix.at<double>(neighbor, pose) =
            penalizing_factor *
            distance_matrix.at<double>(neighbor, pose);  // symmetrical-Matrix --> saves half the computation time
      }
    }
  }
  ...
  for (int start = 0; start < chosen_positions.size(); ++start)
  {
    ...
    for (size_t vertex = 0; vertex < current_order.size(); ++vertex)
      current_pathlength +=
          distance_matrix.at<double>(current_order[vertex], current_order[(vertex + 1) % current_order.size()]);
  ...
  for (std::vector<cv::Point>::iterator position = chosen_positions.begin(); position != chosen_positions.end();
       ++position)
  {
  ...
  std::vector<int>::iterator start =
      std::find(best_order.begin(), best_order.end(),
                starting_index);  // obtain iterator to index in best order to start path creation from there
  for (size_t pose = start - best_order.begin();
       path.size() != chosen_poses.size() && fov_poses.size() != chosen_poses.size(); ++pose)
  {
