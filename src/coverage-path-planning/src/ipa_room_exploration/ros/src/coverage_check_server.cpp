...
CoverageCheckServer::CoverageCheckServer(ros::NodeHandle nh) : node_handle_(nh)
{
...
bool CoverageCheckServer::checkCoverage(ipa_building_msgs::CheckCoverageRequest& request,
                                        ipa_building_msgs::CheckCoverageResponse& response)
{
  ...
  bool return_value = checkCoverage(map, request.map_resolution,
                                    cv::Point2d(request.map_origin.position.x, request.map_origin.position.y), path,
                                    field_of_view, fov_origin, request.coverage_radius, request.check_for_footprint,
                                    request.check_number_of_coverages, coverage_map, number_of_coverage_image);
...
bool CoverageCheckServer::checkCoverage(const cv::Mat& map, const float map_resolution, const cv::Point2d& map_origin,
                                        const std::vector<cv::Point3d>& path,
                                        const std::vector<Eigen::Matrix<float, 2, 1> >& field_of_view,
                                        const Eigen::Matrix<float, 2, 1>& fov_origin, const float coverage_radius,
                                        const bool check_for_footprint, const bool check_number_of_coverages,
...
void CoverageCheckServer::drawCoveredPointsPolygon(cv::Mat& reachable_areas_map,
                                                   const std::vector<cv::Point3d>& robot_poses,
                                                   const std::vector<Eigen::Matrix<float, 2, 1> >& field_of_view,
                                                   const Eigen::Matrix<float, 2, 1>& fov_origin,
                                                   ...
                                                   cv::Mat* number_of_coverages_image)
{
  ...
  for (std::vector<cv::Point3d>::const_iterator current_pose = robot_poses.begin(); current_pose != robot_poses.end();
       ++current_pose)
  {
    ...
    for (size_t point = 0; point < field_of_view.size(); ++point)
    {
      ...
      // values
      transformed_fov_points.push_back(
          clampImageCoordinates(cv::Point((transformed_fov_point(0, 0) - map_origin.x) * map_resolution_inverse,
                                          (transformed_fov_point(1, 0) - map_origin.y) * map_resolution_inverse),
                                reachable_areas_map.rows, reachable_areas_map.cols));
    }

    // transform field of view origin
    const Eigen::Matrix<float, 2, 1> transformed_fov_origin = pose_as_matrix + R * fov_origin;
    const cv::Point transformed_fov_origin_point =
        clampImageCoordinates(cv::Point((transformed_fov_origin(0, 0) - map_origin.x) * map_resolution_inverse,
                                        (transformed_fov_origin(1, 0) - map_origin.y) * map_resolution_inverse),
                              reachable_areas_map.rows, reachable_areas_map.cols);
...
void CoverageCheckServer::drawCoveredPointsCircle(cv::Mat& reachable_areas_map,
                                                  const std::vector<cv::Point3d>& robot_poses,
                                                  ...
                                                  const cv::Point2d map_origin, cv::Mat* number_of_coverages_image)
{
  ...
  cv::Mat map_copy =
      reachable_areas_map.clone();  // copy to draw positions that get later drawn into free space of the original map
  ...
  for (std::vector<cv::Point3d>::const_iterator pose = robot_poses.begin(); pose != robot_poses.end(); ++pose)
  {
    ...
    cv::Point current_point((pose->x - map_origin.x) * map_resolution_inverse,
                            (pose->y - map_origin.y) * map_resolution_inverse);
