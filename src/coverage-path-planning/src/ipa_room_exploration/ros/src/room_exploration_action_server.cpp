...
RoomExplorationServer::RoomExplorationServer(ros::NodeHandle nh, std::string name_of_the_action)
  : node_handle_(nh)
  , room_exploration_server_(node_handle_, name_of_the_action,
                             boost::bind(&RoomExplorationServer::exploreRoom, this, _1), false)
{
  // dynamic reconfigure
  room_exploration_dynamic_reconfigure_server_.setCallback(
      boost::bind(&RoomExplorationServer::dynamic_reconfigure_callback, this, _1, _2));
  ...
  node_handle_.param("file_name", file_name, std::string("path_file"));
  std::cout << "path willbe saved in  " << file_name << std::endl;
...
void RoomExplorationServer::dynamic_reconfigure_callback(ipa_room_exploration::RoomExplorationConfig& config,
                                                         uint32_t level)
{
  ...
  std::cout << "room_exploration/map_correction_closing_neighborhood_size_ = "
            << map_correction_closing_neighborhood_size_ << std::endl;
...
void RoomExplorationServer::exploreRoom(const ipa_building_msgs::RoomExplorationGoalConstPtr& goal)
{
  ...
  for (uint16_t v = 0; v < room_map.rows; ++v)
    for (uint16_t u = 0; u < room_map.cols; ++u)
  ...
  double grid_spacing_in_meter = 0.0;  // is the square grid cell side length that fits into the circle with the robot's
                                       // coverage radius or fov coverage radius
  float fitting_circle_radius_in_meter = 0;
  Eigen::Matrix<float, 2, 1> fitting_circle_center_point_in_meter;  // this is also considered the center of the field
                                                                    // of view, because around this point the maximum
                                                                    // radius incircle can be found that is still inside
                                                                    // the fov
  ...
  {
    // Get the size of one grid cell s.t. the grid can be completely covered by the field of view (fov) from all
    // rotations around it.
    for (int i = 0; i < 4; ++i)
  ...
  const double grid_spacing_in_pixel =
      grid_spacing_in_meter / map_resolution;  // is the square grid cell side length that fits into the circle with the
                                               // robot's coverage radius or fov coverage radius, multiply with sqrt(2)
                                               // to receive the whole working width
  ...
  if (room_exploration_algorithm_ == 1)  // use grid point explorator
  {
    ...
    if (planning_mode_ == PLAN_FOR_FOV)
      grid_point_planner.getExplorationPath(room_map, exploration_path, map_resolution, starting_position, map_origin,
                                            std::floor(grid_spacing_in_pixel), false,
                                            fitting_circle_center_point_in_meter, tsp_solver_, tsp_solver_timeout_);
    else
      grid_point_planner.getExplorationPath(room_map, exploration_path, map_resolution, starting_position, map_origin,
                                            std::floor(grid_spacing_in_pixel), true, zero_vector, tsp_solver_,
                                            tsp_solver_timeout_);
  }
  else if (room_exploration_algorithm_ == 2)  // use boustrophedon explorator
  {
    ...
    if (planning_mode_ == PLAN_FOR_FOV)
      boustrophedon_explorer_.getExplorationPath(room_map, exploration_path, map_resolution, starting_position,
                                                 ...
                                                 cell_visiting_order_, false, fitting_circle_center_point_in_meter,
  ...
  {
    if (planning_mode_ == PLAN_FOR_FOV)
      energy_functional_explorator_.getExplorationPath(room_map, exploration_path, map_resolution, starting_position,
                                                       map_origin, grid_spacing_in_pixel, false,
                                                       fitting_circle_center_point_in_meter);
    else
      energy_functional_explorator_.getExplorationPath(room_map, exploration_path, map_resolution, starting_position,
                                                       map_origin, grid_spacing_in_pixel, true, zero_vector);
  }
  ...
  else if (room_exploration_algorithm_ == 8)  // use boustrophedon variant explorator
  {
    // plan path
    if (planning_mode_ == PLAN_FOR_FOV)
      boustrophedon_variant_explorer_.getExplorationPath(
          room_map, exploration_path, map_resolution, starting_position, map_origin, grid_spacing_in_pixel,
          grid_obstacle_offset_, path_eps_, cell_visiting_order_, false, fitting_circle_center_point_in_meter,
          min_cell_area_, max_deviation_from_track_);
    else
  ...
  if (return_path_ == true)
  {
    ...
    if (client.call(srv))
    {
      ROS_INFO("Path saved from room exploration server");
    }
    else
    {
      ROS_ERROR("Failed to call path save service ");
    }
  ...
  {
    navigateExplorationPath(exploration_path, goal->field_of_view, goal->field_of_view_origin, goal->coverage_radius,
                            fitting_circle_center_point_in_meter.norm(), map_resolution, goal->map_origin,
                            grid_spacing_in_pixel, room_map.rows * map_resolution);
...
// remove unconnected, i.e. inaccessible, parts of the room (i.e. obstructed by furniture), only keep the room with the
// largest area
bool RoomExplorationServer::removeUnconnectedRoomParts(cv::Mat& room_map)
{
  ...
  std::map<uint16_t, uint16_t> area_to_label_map;  // maps area=number of segment pixels (keys) to the respective label
                                                   // (value)
  uint16_t label = 1;
  for (uint16_t v = 0; v < room_map_int.rows; ++v)
  {
    for (uint16_t u = 0; u < room_map_int.cols; ++u)
    {
...
void RoomExplorationServer::downsampleTrajectory(const std::vector<geometry_msgs::Pose2D>& path_uncleaned,
...
void RoomExplorationServer::navigateExplorationPath(const std::vector<geometry_msgs::Pose2D>& exploration_path,
                                                    ...
                                                    const geometry_msgs::Point32& field_of_view_origin,
                                                    const double coverage_radius,
                                                    const double distance_robot_fov_middlepoint,
                                                    const float map_resolution, const geometry_msgs::Pose& map_origin,
                                                    const double grid_spacing_in_pixel, const double map_height)
{
  ...
  for (size_t map_oriented_pose = 0; map_oriented_pose < exploration_path.size(); ++map_oriented_pose)
  {
    ...
    publishNavigationGoal(pose, map_frame_, camera_frame_, robot_poses, distance_robot_fov_middlepoint, temp_goal_eps,
                          true);  // eps = 0.35
    last_pose = pose;
  }
  ...
  if (revisit_areas_ == true)
  {
    ...
    if (coverage_checker.checkCoverage(costmap_as_mat, map_resolution,
                                       cv::Point2d(map_origin.position.x, map_origin.position.y), path, fov, fov_origin,
                                       coverage_radius, (planning_mode_ == PLAN_FOR_FOOTPRINT), false, coverage_map,
    ...
    // ***************** IV. Find leftover areas and lay a grid over it, then plan a path trough all grids s.t. they can
    // be covered by the fov. *****************
    ...
    for (size_t area = 0; area < left_areas.size(); ++area)
    {
      ...
      if (hierarchy[area][3] == -1)
      {
        ...
        // subtract the area from the hole contours inside the found contour, because the contour area grows extremly
        // large if it is a closed loop
    ...
    std::vector<int> revisiting_order =
        tsp_solver.solveConcordeTSP(costmap_as_mat, area_centers, 0.25, 0.0, map_resolution, min_index, 0);

    // 5. go to each center and use the map_accessability_server to find a robot pose around it s.t. it can be covered
    // by the field of view or robot center
    const double pi_8 = PI / 8;
    const std::string perimeter_service_name = "/room_exploration/map_accessibility_analysis/"
                                               "map_perimeter_accessibility_check";
    //	robot_poses.clear();
    for (size_t center = 0; center < revisiting_order.size(); ++center)
    {
      ...
      std::cout << "checking center: " << std::endl
                << current_center << "radius: " << check_request.radius << std::endl;
      ...
      if (ros::service::call(perimeter_service_name, check_request, response) == true)
      {
        ...
        for (std::vector<geometry_msgs::Pose2D>::iterator pose = response.accessible_poses_on_perimeter.begin();
             pose != response.accessible_poses_on_perimeter.end(); ++pose)
...
// The function tracks the robot pose while moving to the goal and adds these poses to the given pose-vector. This is
// done
// because it allows to calculate where the robot field of view has theoretically been and identify positions of the map
// that
...
bool RoomExplorationServer::publishNavigationGoal(const geometry_msgs::Pose2D& nav_goal, const std::string map_frame,
                                                  ...
                                                  std::vector<geometry_msgs::Pose2D>& robot_poses,
                                                  const double robot_to_fov_middlepoint_distance, const double eps,
                                                  const bool perimeter_check)
{
  ...
  std::cout << "navigation goal: (" << map_oriented_pose.x << ", " << map_oriented_pose.y << ", "
            << map_oriented_pose.theta << ")" << std::endl;
  ...
  do
  {
    ...
    try
    {
      ...
      if ((current_pose.x - map_oriented_pose.x) * (current_pose.x - map_oriented_pose.x) +
              (current_pose.y - map_oriented_pose.y) * (current_pose.y - map_oriented_pose.y) <=
          eps * eps)
        near_pos = true;
    ...
    {
      ROS_WARN_STREAM("Couldn't get transform from " << camera_frame << " to " << map_frame
                                                     << "!");  // %s", ex.what());
    }

  } while (mv_base_client.getState() != actionlib::SimpleClientGoalState::ABORTED &&
           mv_base_client.getState() != actionlib::SimpleClientGoalState::SUCCEEDED && near_pos == false);
