...
void calculate_basic_measures(const cv::Mat& map, const double map_resolution, const int number_rooms,
                              std::vector<double>& areas, std::vector<double>& perimeters,
                              std::vector<double>& area_perimeter_compactness, std::vector<double>& bb_area_compactness,
...
// calculate area for every room
std::vector<double> calculate_areas_from_segmented_map(const cv::Mat& map, const double map_resolution,
                                                       const int number_rooms)
{
  ...
  for (int p = 0; p < edge_points.size(); p++)
  {
    for (int np = 0; np < edge_points.size(); np++)
    {
      if (std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) +
                    std::pow((edge_points[p].y - edge_points[np].y), 2)) > distance)
      {
...
int main(int argc, char** argv)
{
  ...
  const std::string segmented_map_path =
      "room_segmentation/";  // ros::package::getPath("ipa_room_segmentation") + "/common/files/segmented_maps/";
  ...
  // - fit with giving ground truth (average of recalls, average recall, average of precisions, average precision) [rows
  // 22-25]
  ...
  for (size_t image_index = 0; image_index < map_names.size(); ++image_index)
  {
    ...
    std::vector<double> av_area_vector(segmentation_names.size()), max_area_vector(segmentation_names.size()),
        min_area_vector(segmentation_names.size()), dev_area_vector(segmentation_names.size());
    std::vector<double> av_per_vector(segmentation_names.size()), max_per_vector(segmentation_names.size()),
        min_per_vector(segmentation_names.size()), dev_per_vector(segmentation_names.size());
    std::vector<double> av_compactness_vector(segmentation_names.size()),
        max_compactness_vector(segmentation_names.size()), min_compactness_vector(segmentation_names.size()),
        dev_compactness_vector(segmentation_names.size());
    std::vector<double> av_bb_vector(segmentation_names.size()), max_bb_vector(segmentation_names.size()),
        min_bb_vector(segmentation_names.size()), dev_bb_vector(segmentation_names.size());
    std::vector<double> av_quo_vector(segmentation_names.size()), max_quo_vector(segmentation_names.size()),
        min_quo_vector(segmentation_names.size()), dev_quo_vector(segmentation_names.size());
    std::vector<bool> reachable(segmentation_names.size());

    // load map
    std::string map_name = map_names[image_index];
    std::string image_filename =
        ros::package::getPath("ipa_room_segmentation") + "/common/files/test_maps/" + map_name + ".png";
    ...
    {
      std::cout << "Evaluating image '" << map_name << "' with segmentation method "
                << segmentation_names[segmentation_index] << std::endl;
      ...
      DynamicReconfigureClient drc(n, "room_segmentation_server/set_parameters", "room_segmentation_server/"
                                                                                 "parameter_updates");
      ...
      cv_bridge::CvImagePtr cv_ptr_seq =
          cv_bridge::toCvCopy(result->segmented_map, sensor_msgs::image_encodings::TYPE_32SC1);
      ...
      std::string image_filename =
          segmented_map_path + map_name + "_segmented_" + segmentation_names[segmentation_index] + ".png";
      ...
      std::vector<double> pca_eigenvalue_ratio;
      calculate_basic_measures(segmented_map, map_resolution, (int)result->room_information_in_pixel.size(), areas,
                               perimeters, area_perimeter_compactness, bb_area_compactness, pca_eigenvalue_ratio);

      // runtime
      results[segmentation_index].at<double>(0, image_index) = runtime;

      // number of segments
      results[segmentation_index].at<double>(1, image_index) = segments_number_vector[segmentation_index] =
          areas.size();

      // area
      // std::vector<double> areas = calculate_areas_from_segmented_map(segmented_map,
      // (int)result->room_information_in_pixel.size());
      double average = 0.0;
      double max_area = 0.0;
      ...
      results[segmentation_index].at<double>(5, image_index) = dev_area_vector[segmentation_index] =
          calculate_stddev(areas, average);
      ...
      results[segmentation_index].at<double>(9, image_index) = dev_per_vector[segmentation_index] =
          calculate_stddev(perimeters, average);
      ...
      results[segmentation_index].at<double>(11, image_index) = max_compactness_vector[segmentation_index] =
          max_compactness;
      results[segmentation_index].at<double>(12, image_index) = min_compactness_vector[segmentation_index] =
          min_compactness;
      results[segmentation_index].at<double>(13, image_index) = dev_compactness_vector[segmentation_index] =
          calculate_stddev(area_perimeter_compactness, average);
      ...
      results[segmentation_index].at<double>(17, image_index) = dev_bb_vector[segmentation_index] =
          calculate_stddev(bb_area_compactness, average);
      ...
      results[segmentation_index].at<double>(21, image_index) = dev_quo_vector[segmentation_index] =
          calculate_stddev(pca_eigenvalue_ratio, average);
      ...
      // =============================================
      // load ground truth segmentation (just borders painted in between of rooms/areas, not colored yet --> coloring
      // will be done here)
      ...
      es.computePrecisionRecall(gt_map, gt_map_color, segmented_map, precision_micro, precision_macro, recall_micro,
                                recall_macro, true);
      std::string gt_image_filename_color =
          segmented_map_path + map_name +
          "_gt_color_segmentation.png";  // ros::package::getPath("ipa_room_segmentation") + "/common/files/test_maps/"
                                         // + map_name + "_gt_color_segmentation.png";
    ...
    for (size_t segmentation_index = 0; segmentation_index < segmentation_names.size(); ++segmentation_index)
    {
      std::string log_filename = segmented_map_path + segmentation_names[segmentation_index] + "_evaluation_summary."
                                                                                               "txt";
      std::ofstream file(log_filename.c_str(), std::ios::out);
      if (file.is_open() == true)
      {
