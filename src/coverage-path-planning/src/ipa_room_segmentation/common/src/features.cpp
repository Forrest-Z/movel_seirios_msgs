...
double LaserScannerFeatures::get_feature(const std::vector<double>& beams, const std::vector<double>& angles,
                                         cv::Point point, const int feature)
{
  switch (feature)
  {
    case 1:
...
void LaserScannerFeatures::get_features(const std::vector<double>& beams, const std::vector<double>& angles,
                                        cv::Point point, cv::Mat& features)
{
...
std::vector<cv::Point> LaserScannerFeatures::calc_polygonal_approx(const std::vector<double>& beams,
...
cv::Point LaserScannerFeatures::calc_centroid(const std::vector<double>& beams, const std::vector<double>& angles,
                                              cv::Point location)
{
...
double LaserScannerFeatures::calc_feature14(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
...
double LaserScannerFeatures::calc_feature15(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
...
double LaserScannerFeatures::calc_feature16(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
...
// approximation
double LaserScannerFeatures::calc_feature17(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
...
double LaserScannerFeatures::calc_feature18(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
  ...
  for (int p = 0; p < polygon.size(); p++)
  {
    ...
    current_distance =
        std::sqrt(delta_x * delta_x + delta_y * delta_y);          // std::pow(delta_x, 2) + std::pow(delta_y, 2));
...
double LaserScannerFeatures::calc_feature19(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
  ...
  for (int p = 0; p < edge_points.size(); p++)
  {
    ...
    {
      // if (std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) + std::pow((edge_points[p].y -
      // edge_points[np].y), 2)) > distance)
      const float a = (edge_points[p].x - edge_points[np].x);
      const float b = (edge_points[p].y - edge_points[np].y);
      const double sqr = a * a + b * b;
      if (sqr > distance)
      {
        // distance = std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) + std::pow((edge_points[p].y -
        // edge_points[np].y), 2));
        distance = sqr;
      }
    }
...
double LaserScannerFeatures::calc_feature20(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
  ...
  for (int p = 0; p < edge_points.size(); p++)
  {
    for (int np = 0; np < edge_points.size(); np++)
    {
      ...
      //			if (std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) + std::pow((edge_points[p].y -
      //edge_points[np].y), 2)) < distance
      //			        && std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) + std::pow((edge_points[p].y -
      //edge_points[np].y), 2)) > 0 && p != np)
      const float a = (edge_points[p].x - edge_points[np].x);
      const float b = (edge_points[p].y - edge_points[np].y);
      const double sqr = a * a + b * b;
...
double LaserScannerFeatures::calc_feature21(const std::vector<double>& beams, const std::vector<double>& angles,
                                            cv::Point location)
{
