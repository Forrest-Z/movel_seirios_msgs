                                              ...
                                              const cv::Scalar voronoi_color, const cv::Mat& eroded_map)
{
       ...
       current_contour != facets_of_voronoi.end(); ++current_contour)
  {
    ...
    for (size_t c = 0; c < current_contour->size(); ++c)
    {
      ...
      if (((int)current_point.x < 0) || ((int)current_point.x >= eroded_map.cols) || ((int)current_point.y < 0) ||
          ((int)current_point.y >= eroded_map.rows) ||
          eroded_map.at<uchar>((int)current_point.y, (int)current_point.x) == 0 || ((int)last_point.x < 0) ||
          ((int)last_point.x >= eroded_map.cols) || ((int)last_point.y < 0) || ((int)last_point.y >= eroded_map.rows) ||
...
{
  cv::Mat map_to_draw_voronoi_in =
      map_for_voronoi_generation.clone();  // variable to save the given map for drawing in the voronoi-diagram

  cv::Mat temporary_map_to_calculate_voronoi =
      map_for_voronoi_generation.clone();  // variable to save the given map in the createVoronoiGraph-function
  ...
  cv::Rect rect(0, 0, map_to_draw_voronoi_in.cols, map_to_draw_voronoi_in.rows);  // variables to generate the
                                                                                  // voronoi-diagram, using OpenCVs
                                                                                  // delaunay-triangulation
  cv::Subdiv2D subdiv(rect);
  std::vector<std::vector<cv::Point> > hole_contours;  // variable to save the hole-contours (= black holes inside the
                                                       // white map)
  ...
  std::vector<std::vector<cv::Point2f> > voronoi_facets;  // variables to find the facets and centers of the
                                                          // voronoi-cells
...
void AbstractVoronoiSegmentation::pruneVoronoiGraph(cv::Mat& voronoi_map,
                                             ...
                                             bool display_map)
{
  ...
  for (int y = 0; y < map_to_merge_rooms.rows; y++)
  {
    for (int x = 0; x < map_to_merge_rooms.cols; x++)
    {
      ...
      if (current_id != 0)
      {
        for (size_t current_room = 0; current_room < rooms.size();
             current_room++)  // add the Points with the same Id as a room to it
        {
  ...
  for (int current_room_index = 0; current_room_index < rooms.size();)
  {
    ...
    if (current_room.getNeighborCount() == 1 && current_room.getArea() < max_area_for_merging &&
  ...
  for (int current_room_index = 0; current_room_index < rooms.size();)
  {
        ...
        (double)current_room.getNeighborStatistics()[max_border_neighbor_id] / current_room.getPerimeter() > 0.15)
    {
      ...
      if (rooms[merge_index].getNeighborCount() > 2 &&
          rooms[merge_index].getPerimeterRatioOfXLargestRooms(2) <
              0.95)  // || rooms[merge_index].getWallToPerimeterRatio() < 0.4)
        merge_rooms = false;
    }
         ...
         it != neighbor_room_statistics_inverse.end(); ++it)
    {
           ...
           current_room.getWallToPerimeterRatio() < (1 - neighbor_border_ratio)))
      {
        ...
        if ((double)rooms[merge_index].getNeighborStatistics()[current_room.getID()] /
                rooms[merge_index].getPerimeter() <=
            0.1)
          merge_rooms = false;
        if (merge_rooms == true)
          break;
