...
void MorphologicalSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map,
                                           ...
                                           double room_area_factor_upper_limit)
{
  ...
  std::vector<std::vector<cv::Point> > saved_contours;  // saving variable for every contour that is between the upper
                                                        // and the lower limit
    ...
    if (temporary_contours.size() != 0)
    {
      ...
      {  // only take first level contours --> second level contours belong to holes and doesn't need to be looked at
        if (hierarchy[current_contour][3] == -1)
        {
          ...
          {
            if (hierarchy[hole][3] ==
                current_contour)  // check if the parent of the hole is the current looked at contour
            {
          ...
          if (room_area_factor_lower_limit < room_area && room_area < room_area_factor_upper_limit)
          {
            ...
            cv::drawContours(temporary_map_to_find_rooms, temporary_contours, current_contour, cv::Scalar(0),
                             cv::FILLED, 8, hierarchy, 2);
          }
        }
      }
    }
