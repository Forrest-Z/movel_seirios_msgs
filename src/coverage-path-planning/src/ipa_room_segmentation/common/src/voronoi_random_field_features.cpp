...
double voronoiRandomFieldFeatures::getFeature(const std::vector<double>& beams, const std::vector<double>& angles,
                                              const std::vector<cv::Point>& clique_points,
                                              std::vector<unsigned int>& labels_for_clique_points,
                                              std::vector<unsigned int>& possible_labels, cv::Point point,
                                              const int feature)
{
  switch (feature)
  {
...
void voronoiRandomFieldFeatures::getFeatures(const std::vector<double>& beams, const std::vector<double>& angles,
                                             const std::vector<cv::Point>& clique_points,
                                             std::vector<unsigned int>& labels_for_clique_points,
                                             std::vector<unsigned int>& possible_labels, cv::Point point,
...
// Calculate Feature 12: The number of relative gaps. A relative gap is when the relation (b_i/b_(i+1)) is smaller than
// a
// specified threshold
...
std::vector<cv::Point> voronoiRandomFieldFeatures::calcPolygonalApprox(const std::vector<double>& beams,
...
cv::Point voronoiRandomFieldFeatures::calcCentroid(const std::vector<double>& beams, const std::vector<double>& angles,
                                                   cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature14(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature15(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature16(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
// Calculate Feature 17: The average of the distance between the centroid and the boundary-Points of the polygonal
// approximation
double voronoiRandomFieldFeatures::calcFeature17(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature18(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature19(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature20(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
  ...
  for (int p = 0; p < edge_points.size(); p++)
  {
    for (int np = 0; np < edge_points.size(); np++)
    {
      if (std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) +
                    std::pow((edge_points[p].y - edge_points[np].y), 2)) < distance &&
          std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) +
                    std::pow((edge_points[p].y - edge_points[np].y), 2)) > 0 &&
          p != np)
      {
        distance = std::sqrt(std::pow((edge_points[p].x - edge_points[np].x), 2) +
                             std::pow((edge_points[p].y - edge_points[np].y), 2));
...
double voronoiRandomFieldFeatures::calcFeature21(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 cv::Point location)
{
...
// Calculate Feature 24: The curvature of the voronoi graph approximated by the points of the clique. The curvature of a
// graph
...
double voronoiRandomFieldFeatures::calcFeature24(std::vector<cv::Point> clique_points)
{
  ...
  // Get the circle that approaches the points of the clique. If the clique has more than three neighbors it is a
  // voronoi-node
...
// Calculate Feature 25: The relation between the labels of Points from the central point to the other points in the
// clique.
...
double voronoiRandomFieldFeatures::calcFeature25(std::vector<unsigned int>& possible_labels,
                                                 std::vector<unsigned int>& labels_for_points)
{
  ...
  // set the initial value for the feature (40 * maximal_amount, so the feature is always >= 0, even if later a lot of
  // the value gets subtracted)
  double feature_value = 40 * maximal_amount;

  // ***Check for possibility that two neighboring labels can occur. For example it is very unlikely that a hallway
  // appears right
  // after a room, without a doorway between, but it is very likely that a hallway adds to a hallway.

  // Create a map to get a factor to add or subtract from the inital-value for a feature. The key for this map is the
  // sum of
  ...
  // create each possible key and data (done by hand because in this case all possible configurations are known before
  // --> room,hallway,doorway)
  ...
  for (std::vector<unsigned int>::iterator current_point = labels_for_points.begin();
       current_point != labels_for_points.end(); ++current_point)
  {
    // check each neighbor that isn't the point itself
    for (std::vector<unsigned int>::iterator current_neighbor = labels_for_points.begin();
         current_neighbor != labels_for_points.end(); ++current_neighbor)
    {
...
// Feature 27: The are of the bounding box that is calculated for the endpoints of all beams that are limited to a
// maxval
double voronoiRandomFieldFeatures::calcFeature27(const std::vector<double>& beams, const std::vector<double>& angles,
                                                 double epsilon, cv::Point location)
{
...
double voronoiRandomFieldFeatures::calcFeature28(const std::vector<double>& beams, double number_of_beams)
{
  ...
  int index = 0;
  for (std::vector<double>::iterator beam = sorted_beams.begin(); beam != sorted_beams.begin() + number_of_beams;
       ++beam)
  {
  ...
  index = 0;
  for (std::vector<double>::reverse_iterator beam = sorted_beams.rbegin();
       beam != sorted_beams.rbegin() + number_of_beams; ++beam)
  {
  ...
  double average_longest =
      std::accumulate(longest_beams.begin(), longest_beams.end(), 0) / (double)longest_beams.size();
  double average_shortest =
      std::accumulate(shortest_beams.begin(), shortest_beams.end(), 0) / (double)shortest_beams.size();
