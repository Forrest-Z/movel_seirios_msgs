...
void AdaboostClassifier::trainClassifiers(const std::vector<cv::Mat>& room_training_maps,
                                          const std::vector<cv::Mat>& hallway_training_maps,
                                          const std::string& classifier_storage_path)
{
  ...
  hallway_boost_.train(hallway_features_mat, CV_ROW_SAMPLE, hallway_labels_mat, cv::Mat(), cv::Mat(), cv::Mat(),
                       cv::Mat(), params_);
...
#if CV_MAJOR_VERSION == 2
  // Train a boost classifier
  room_boost_.train(room_features_mat, CV_ROW_SAMPLE, room_labels_mat, cv::Mat(), cv::Mat(), cv::Mat(), cv::Mat(),
                    params_);
  // save the trained booster
...
void AdaboostClassifier::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map,
                                    double map_resolution_from_subscription, double room_area_factor_lower_limit,
                                    double room_area_factor_upper_limit, const std::string& classifier_storage_path,
                                    const std::string& classifier_default_path, bool display_results)
{
  //******************Semantic-labeling function based on AdaBoost*****************************
  // This function calculates single-valued features for every white Pixel in the given occupancy-gridmap and classifies
  // it
  ...
  //	II. Go trough each Pixel of the given map. If this Pixel is white simulate the laser-beams for it and calculate
  //each
  ...
  //		only room-areas are shown in the map and find them. Then make these room-areas black and finally set the
  //threshold
  //		a little lower than the hallway-colour. The function only takes contours that are larger than the minimum value
  //		and splits too large hallway-areas into smaller areas by putting random Points into the too large hallway
  //contour
...
// classify each Point
        ...
        double room_certanity =
            (std::exp((double)room_sum)) / (std::exp(-1 * (double)room_sum) + std::exp((double)room_sum));
  ...
  cv::drawContours(blured_image_for_thresholding, contours, -1, cv::Scalar(0), cv::FILLED);  // make the found regions
                                                                                             // at the original map
                                                                                             // black, because they have
                                                                                             // been looked at already

  // only take rooms that are large enough and that are not a hole-contour
  for (int c = 0; c < contours.size(); c++)
  {
    if (map_resolution_from_subscription * map_resolution_from_subscription * cv::contourArea(contours[c]) >
  ...
  {
    if (map_resolution_from_subscription * map_resolution_from_subscription *
            cv::contourArea(contours[contour_counter]) >
        room_area_factor_upper_limit)
    {
      ...
      } while (center_counter <= (map_resolution_from_subscription * map_resolution_from_subscription *
                                  cv::contourArea(contours[contour_counter])) /
                                     8);
      cv::Mat temporary_Map_to_wavefront;
      contour_Map.convertTo(temporary_Map_to_wavefront, CV_32SC1, 256, 0);
      // draw the centers as white circles into a black map and give the center-map and the contour-map to the opencv
      // watershed-algorithm
      for (int current_center = 0; current_center < temporary_watershed_centers.size(); current_center++)
      {
        ...
        do
        {
          ...
          {
            cv::circle(temporary_Map_to_wavefront, temporary_watershed_centers[current_center], 2, fill_colour,
                       cv::FILLED);
    ...
    else if (map_resolution_from_subscription * map_resolution_from_subscription *
                 cv::contourArea(contours[contour_counter]) >
             room_area_factor_lower_limit)
    {
