...
void DistanceSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map,
                                      ...
                                      double room_area_factor_upper_limit)
{
  ...
  double constant_additional_value =
      optimal_room_area *
      optimal_room_area;  // variable that sets the energy function higher so that it is 0 for the lower limit
  ...
  cv::Mat distance_map;  // variable for the distance-transformed map, type: cv::32FC1
  ...
  // 2. Threshold the map and find the contours of the rooms. Change the threshold and repeat steps until last possible
  // threshold.
  // Then take the contours from the threshold with the most contours between the roomfactors and draw it in the map
  // with a random color.
  ...
  {  // change the threshold for the grayscale-image from largest possible value to smallest
    ...
    // Energy function: -(x-a)^2 + b, where x is the current area, a is the optimal area and b is a factor to make the
    // function zero at x=0
    for (int c = 0; c < contours.size(); c++)
    {
      if (hierarchy[c][3] == -1)
      {
        double room_area =
            map_resolution_from_subscription * map_resolution_from_subscription * cv::contourArea(contours[c]);
        ...
        for (int hole = 0; hole < contours.size(); hole++)
        {
          ...
          {
            room_area -=
                map_resolution_from_subscription * map_resolution_from_subscription * cv::contourArea(contours[hole]);
          }
        }
