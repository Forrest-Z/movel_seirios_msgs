...
void EvaluationSegmentation::computePrecisionRecall(const cv::Mat& gt_map, cv::Mat& gt_map_color,
                                                    const cv::Mat& segmented_map, double& precision_micro,
                                                    double& precision_macro, double& recall_micro, double& recall_macro,
                                                    bool compute_gt_map_color)
{
  ...
  else
  {
    ...
    std::map<int, PointSet> gt_points_map;  // maps a label key identifier to a vector of points belonging to that room
                                            // label
  ...
  // get point sets for segmentation map
  std::map<int, PointSet> seg_points_map;  // maps a label key identifier to a vector of points belonging to that room
                                           // label
  ...
  for (size_t v = 0; v < seg_points_vector.size(); ++v)
  {
    ...
    {
      int overlapping = 0;
      for (PointSet::iterator it = seg_points_vector[v].begin(); it != seg_points_vector[v].end();
           it++)  //       size_t pv=0; pv<seg_points_vector[v].size(); ++pv)
      {
        if (gt_points_vector[u].find(*it) !=
            gt_points_vector[u].end())  //         std::find(gt_points_vector[u].begin(), gt_points_vector[u].end(),
                                        //         seg_points_vector[v][pv]) != gt_points_vector[u].end())
          ++overlapping;
      }
  ...
  // std::cout << recall_micro << "\t" << precision_micro << "\t" << recall_macro << "\t" << precision_macro <<
  // std::endl;
}

// void EvaluationSegmentation::Segmentation_Vector_calculation(const cv::Mat &segmented_map, std::map<cv::Point2i,
// cv::Vec3b> &segmented_room_mapping)
//{
//	for(unsigned int i=0; i <segmented_map.cols ; i++)
//	{
//		for(unsigned int j=0; j <segmented_map.rows ; j++)
//		{
//			if (segmented_map.at<cv::Vec3b>(i,j)[0] != 0 && segmented_map.at<cv::Vec3b>(i,j)[1] != 0 &&
//segmented_map.at<cv::Vec3b>(i,j)[2] != 0)
//			{
...
//}
//
// void EvaluationSegmentation::Recall_Precision_Calculation(std::map<cv::Point2i, cv::Point2f> &results, std::vector
// <cv::Mat> gt_mat_vector, std::vector <cv::Mat> segmentation_mat_vector)
//{
...
//	map_names.push_back("office_i");
  ...
  const std::string segmented_map_path = "room_segmentation/"; //ros::package::getPath("ipa_room_segmentation") +
"/common/files/segmented_maps/";
...
recall_macro, true);
    std::cout << recall_micro << "\t" << precision_micro << "\t" << recall_macro << "\t" << precision_macro <<
std::endl;
    std::string gt_image_filename_color = segmented_map_path + map_name + "_gt_color_segmentation.png";
//ros::package::getPath("ipa_room_segmentation") + "/common/files/test_maps/" + map_name + "_gt_color_segmentation.png";
...
//
//	for(std::map<cv::Point2i, cv::Vec3b>::iterator iterator = GT_room_mapping.begin(); iterator !=
GT_room_mapping.end(); iterator++)
//	{
