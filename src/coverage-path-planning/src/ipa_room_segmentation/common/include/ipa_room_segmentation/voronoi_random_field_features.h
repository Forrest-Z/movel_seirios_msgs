...
public:
  ...
  void getFeatures(const std::vector<double>& beams, const std::vector<double>& angles,
                   const std::vector<cv::Point>& clique_points, std::vector<unsigned int>& labels_for_clique_points,
  ...
  // feature 7: number of gaps between the beams (a gap is when the difference between the beamlenghts is larger than a
  // threshold)
  ...
  // feature 12: number of relative gaps, a relativ gap is when the relation of two negihboring beams is larger than a
  // threshold
  double calcFeature12(const std::vector<double>& beams);
  // feature 13: Kurtosis = (Sum((x - mean)^4))/sigma^4) - 3, where mean is the mean-value and sigma is the standard
  // deviation
  ...
  std::vector<cv::Point> calcPolygonalApprox(const std::vector<double>&, const std::vector<double>&,
                                             cv::Point location);
  ...
  // feature 27: the area of the bounding box for beams that are smaller than the shortest beam in a defined epsilon
  // neighborhood
  double calcFeature27(const std::vector<double>& beams, const std::vector<double>& angles, double maxval,
                       cv::Point location);
