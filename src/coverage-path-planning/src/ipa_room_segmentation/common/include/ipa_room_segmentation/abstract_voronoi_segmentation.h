...
// Struct that compares two given points and returns if the y-coordinate of the first is smaller or if they are equal if
// the
// x-coordinate of the first is smaller. This is used for sets to easily store cv::Point objects and search for specific
// objects.
...
protected:
  ...
  // This function takes a vector of rooms and an index in this vector and returns the ID of this room, meaning the
  // color it has
  ...
  // This function is used to merge two rooms together in the given already segmented map. It takes two indexes, showing
  // which
  // room is the bigger one and which is the smaller one. The smaller one should be merged together with the bigger one,
  // what is
  // done by changing the color of this room in the map to the color of the bigger room and inserting the members into
  // the bigger
  // room.
  ...
  // Function to draw the generalized voronoi-diagram into a given map, not drawing lines that start or end at black
  // pixels
  // This function draws the Voronoi-diagram into a given map. It needs the facets as vector of Points, the contour of
  // the
  ...
  // Function to draw the approximated voronoi graph into a given map. It doesn't draw lines of the graph that start or
  // end
  // in a black region. This is necessary because the voronoi graph gets approximated by diskretizing the maps contour
  // and
  // using these points as centers for the graph. It gets wrong lines, that are eliminated in this function. See the
  // .cpp
  ...
  void mergeRooms(cv::Mat& map_to_merge_rooms, std::vector<Room>& rooms, double map_resolution_from_subscription,
                  double max_area_for_merging, bool display_map);
