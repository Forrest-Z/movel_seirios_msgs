...
public:
  ...
  void trainClassifiers(const std::vector<cv::Mat>& room_training_maps,
                        const std::vector<cv::Mat>& hallway_training_maps, const std::string& classifier_storage_path);
  ...
  void segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map, double map_resolution_from_subscription,
                  ...
                  const std::string& classifier_storage_path, const std::string& classifier_default_path,
