...
{
public:
  // Struct that compares two given points and returns if the y-coordinate of the first is smaller or if they are equal
  // if the
  // x-coordinate of the first is smaller. This is used for sets to easily store cv::Point objects and search for
  // specific objects.
...
protected:
  ...
  std::map<int, int> neighbor_room_statistics_;  // maps from room labels of neighboring rooms to number of touching
                                                 // pixels of the respective neighboring room
