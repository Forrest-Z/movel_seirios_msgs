...
struct EvaluationConfig
{
  ...
  EvaluationConfig(const int room_segmentation_algorithm, const double max_clique_path_length,
                   const int sequence_planning_method, const int tsp_solver)
  {
  ...
  EvaluationConfig(const int room_segmentation_algorithm, const double max_clique_path_length,
                   const int sequence_planning_method, const int tsp_solver, const double trashbins_per_trolley)
  {
    room_segmentation_algorithm_ = room_segmentation_algorithm;
    max_clique_path_length_ = max_clique_path_length;
    sequence_planning_method_ = sequence_planning_method;
...
struct EvaluationData
{
  ...
  EvaluationData(const std::string& map_name, const cv::Mat& floor_plan, const float map_resolution,
                 const double map_downsampling_factor, const double robot_radius,
                 const std::vector<cv::Point>& trash_bin_locations,
                 const cv::Point trolley_park_position = cv::Point(-1, -1))
  {
    map_name_ = map_name;
...
public:
  ...
  Evaluation(ros::NodeHandle& nh, const std::string& test_map_path, const std::string& data_storage_path,
    ...
    : node_handle_(nh), robot_radius_(robot_radius)
  {
    ...
    for (size_t image_index = 0; image_index < map_names.size(); ++image_index)
    {
      ...
      for (int y = 0; y < temp.rows; y++)
      {
        ...
        {
          // find blue drawn trash bins and check if they are reachable (if one is not reachable the a star planner will
          // give an extremely large path)
          ...
          // find red drawn trolley park and check if they are reachable (if one is not reachable the a star planner
          // will give an extremely large path)
      ...
      //			evaluation_data_.push_back(EvaluationData(map_names[image_index], map, map_resoultion_for_evaluation,
      //0.25, robot_radius_, trash_bin_locations));
      evaluation_data_.push_back(EvaluationData(map_names[image_index], map, map_resoultion_for_evaluation, 0.25,
                                                robot_radius_, trash_bin_locations, central_trolley_park));
    }
    ...
    std::map<std::string, std::vector<StatisticsItem> >
        results;  // maps from [map_name]->StatisticsItems[evaluation_configurations.size()]
  ...
  void readFromLocalFiles(const std::vector<EvaluationConfig>& evaluation_configurations,
                          const std::vector<std::string>& map_names, const std::string& data_storage_path,
                          std::map<std::string, std::vector<StatisticsItem> >& results)
  {
    for (size_t i = 0; i < evaluation_configurations.size(); ++i)
    {
      ...
      for (size_t map_index = 0; map_index < map_names.size(); ++map_index)
      {
        ...
        std::string human_filename = data_storage_path + "human_way/" + upper_folder_name + lower_folder_name +
                                     map_names[map_index] + "_human.txt";
  ...
  void writeGlobalStatistics(const std::vector<EvaluationConfig>& evaluation_configurations,
                             const std::vector<std::string>& map_names, const std::string& data_storage_path,
                             std::map<std::string, std::vector<StatisticsItem> >& results)
  {
    ...
    for (std::vector<std::string>::iterator it_filename = filenames.begin(); it_filename != filenames.end();
         ++it_filename)
    {
      // collect column data
      ...
      for (std::vector<std::string>::const_iterator it = map_names.begin(); it != map_names.end(); it++)
      {
        ...
        int r = 1;
        for (std::set<double>::iterator mcl_it = max_clique_lengths.begin(); mcl_it != max_clique_lengths.end();
             mcl_it++, r++)
        {
          std::stringstream ss;
          ss << "mcl" << *mcl_it;
        ...
        for (int i = 0; i < data_columns; ++i)
        {
          ...
          std::stringstream ss;
          ss << "seg" << evaluation_configurations[base_index].room_segmentation_algorithm_ << "plmth"
             << evaluation_configurations[base_index].sequence_planning_method_ << "tsp"
             << evaluation_configurations[base_index].tsp_solver_;
  ...
  void writeToGlobalFiles(const std::vector<std::string>& map_names, const std::string& data_storage_path,
                          const std::vector<std::vector<double> >& map_pathlengths,
                          const std::vector<std::vector<double> >& map_cleaning_times,
                          const std::vector<std::vector<double> >& sequence_solver_times)
  {
    ...
    for (int max_length = 0; max_length < number_of_cliquelenghts; ++max_length)
    {
      ...
      for (size_t map = 0; map < map_names.size(); ++map)
      {
        ...
        for (size_t segmentation = 0; segmentation < number_of_segmentation_algorithms; ++segmentation)
        {
          ...
          {
            int plmth_start = start +
                              number_of_segmentation_algorithms * number_of_tsp_solver * number_of_cliquelenghts *
                                  sequence_planning_method;
      ...
      for (int sequence_planning_method = 2; sequence_planning_method <= 2; ++sequence_planning_method)
      {
        for (int tsp_solver = 1; tsp_solver <= 3; ++tsp_solver)
        {
          ...
          for (int i = 0; i < max_clique_lengths.cols; ++i)
            evaluation_configurations.push_back(EvaluationConfig(room_segmentation_algorithm,
                                                                 max_clique_lengths.at<double>(0, i),
                                                                 sequence_planning_method, tsp_solver, 10));
        }
      }
    }
  }

  bool evaluateAllConfigs0815(const std::vector<EvaluationConfig>& evaluation_configuration_vector,
                              const EvaluationData& evaluation_data, const std::string& data_storage_path)
  {
    ...
    for (size_t config = 0; config < evaluation_configuration_vector.size(); ++config)
    {
      ...
      const std::string upper_folder_name =
          evaluation_configuration_vector[config].generateUpperConfigurationFolderString() + "/";
      const std::string path = data_storage_path + upper_folder_name;
      const std::string upper_command = "mkdir -p " + path;
      int return_value = system(upper_command.c_str());

      const std::string lower_folder_name =
          evaluation_configuration_vector[config].generateLowerConfigurationFolderString() + "/";
      const std::string lower_path = path + lower_folder_name;
      const std::string lower_command = "mkdir -p " + lower_path;
      return_value = system(lower_command.c_str());

      std::cout << "\nCurrent Configuration:" << std::endl
                << "map: " << evaluation_data.map_name_
                << "\tsegmentation algorithm: " << evaluation_configuration_vector[config].room_segmentation_algorithm_
                ...
                << "\tmaximal clique length: " << evaluation_configuration_vector[config].max_clique_path_length_
                << std::endl;
      ...
      else if (evaluation_configuration_vector[config].room_segmentation_algorithm_ == 4)
      {
        ...
        {
          if (segmentFloorPlan(evaluation_data, evaluation_configuration_vector[config], result_seg_semant, t0) ==
              false)
            return false;
      ...
      Timer tim;
      planner.downsampleMap(evaluation_data.floor_plan_, downsampled_map, evaluation_data.map_downsampling_factor_,
                            evaluation_data.robot_radius_, evaluation_data.map_resolution_);
      std::cout << "downsampling map: " << tim.getElapsedTimeInMilliSec() << " ms" << std::endl;
      cv::Point robot_start_position(
          (evaluation_data.robot_start_position_.position.x - evaluation_data.map_origin_.position.x) /
              evaluation_data.map_resolution_,
          (evaluation_data.robot_start_position_.position.y - evaluation_data.map_origin_.position.y) /
              evaluation_data.map_resolution_);
      ...
      for (size_t i = 0; i < result_seg->room_information_in_pixel.size(); ++i)
      {
        ...
        double length =
            planner.planPath(evaluation_data.floor_plan_, downsampled_map, robot_start_position, current_center,
                             evaluation_data.map_downsampling_factor_, 0., evaluation_data.map_resolution_);
        if (length < 1e9)
      ...
      if (room_centers.size() ==
          0)  // no room center is reachable for the given start position --> needs to be looked at separately
      {
      ...
      if (computeRoomSequence(evaluation_data, evaluation_configuration_vector[config], room_centers, result_seq, t2) ==
          false)
      {
      ...
      {
        int trash_bin_index = segmented_map.at<int>(
            evaluation_data.trash_bin_locations_[i]);  // labeling started from 1 --> 0 is for obstacles
        int room_index = -1;
        ...
        if (room_index != -1)
        {
          ...
          std::cout << "trash bin " << evaluation_data.trash_bin_locations_[i] << "   at room center[" << room_index
                    << "] " << room_centers[room_index] << std::endl;
        }
        else
          std::cout << "########## trash bin " << evaluation_data.trash_bin_locations_[i] << " does not match any room."
                    << std::endl;
      }
      ...
      //			const double max_clique_path_length_in_pixel = evaluation_configuration.max_clique_path_length_ /
      //evaluation_data.map_resolution_;
      cv::Point robot_position = robot_start_position;
      cv::Point trolley_position(
          (evaluation_data.robot_start_position_.position.x - evaluation_data.map_origin_.position.x) /
              evaluation_data.map_resolution_,
          (evaluation_data.robot_start_position_.position.y - evaluation_data.map_origin_.position.y) /
              evaluation_data.map_resolution_);
      ...
      for (size_t clique_index = 0; clique_index < result_seq->checkpoints.size(); ++clique_index)
      {
        ...
        screenoutput << "cleaning new clique" << std::endl;
        // move trolley
        //		i) robot to trolley
        path_length_robot += planner.planPath(evaluation_data.floor_plan_, downsampled_map, robot_position,
                                              trolley_position, evaluation_data.map_downsampling_factor_, 0.,
                                              evaluation_data.map_resolution_, 1, &draw_path_map);
        // 		ii) trolley to next trolley goal
        cv::Point trolley_goal_position(result_seq->checkpoints[clique_index].checkpoint_position_in_pixel.x,
                                        result_seq->checkpoints[clique_index].checkpoint_position_in_pixel.y);
        path_length_trolley += planner.planPath(evaluation_data.floor_plan_, downsampled_map, trolley_position,
                                                trolley_goal_position, evaluation_data.map_downsampling_factor_, 0.,
                                                evaluation_data.map_resolution_, 1, &draw_path_map);
        ...
        cv::circle(draw_path_map, trolley_position, 5, CV_RGB(0, 0, 255), -1);
        cv::circle(draw_path_map2, trolley_position, 5, CV_RGB(0, 0, 255), -1);
        std::cout << "moved trolley to " << trolley_position << std::endl;
        screenoutput << "moved trolley to " << trolley_position << std::endl;
        ...
        for (size_t room = 0; room < result_seq->checkpoints[clique_index].room_indices.size(); ++room)
        {
          ...
          // drive to next room
          path_length_robot += planner.planPath(evaluation_data.floor_plan_, downsampled_map, robot_position,
                                                current_roomcenter, evaluation_data.map_downsampling_factor_, 0.,
                                                evaluation_data.map_resolution_, 1, &draw_path_map);
          ...
          screenoutput << " arrived in room " << current_roomcenter << "\n starting to clean the trash bins"
                       << std::endl;
          ...
          if (room_trash_bins[room_index].size() > 1 &&
              computeTrashBinSequence(evaluation_data, evaluation_configuration_vector[config],
                                      room_trash_bins[room_index], robot_position, result_trash_bin_seq) == true)
          {
            for (size_t cc = 0; cc < result_trash_bin_seq->checkpoints.size(); ++cc)
              for (size_t tt = 0; tt < result_trash_bin_seq->checkpoints[cc].room_indices.size(); ++tt)
                trash_bin_sequence_in_this_room.push_back(
                    room_trash_bins[room_index][result_trash_bin_seq->checkpoints[cc].room_indices[tt]]);
          }
          else
          {
          ...
          for (size_t t = 0; t < trash_bin_sequence_in_this_room.size(); ++t)
          {
            ...
            double trash_bin_dist1 = planner.planPath(
                evaluation_data.floor_plan_, downsampled_map, robot_position, trash_bin_sequence_in_this_room[t],
                evaluation_data.map_downsampling_factor_, 0., evaluation_data.map_resolution_, 1, &draw_path_map);
            ...
            double trash_bin_dist2 =
                2. *
                planner.planPath(evaluation_data.floor_plan_, downsampled_map, trash_bin_sequence_in_this_room[t],
                                 trolley_position, evaluation_data.map_downsampling_factor_, 0.,
                                 evaluation_data.map_resolution_, 1, &draw_path_map2);
            path_length_robot += trash_bin_dist2;
            path_length_trash_bins += trash_bin_dist1 + trash_bin_dist2;
            std::cout << "  room: " << room_index << "\ttrash bin: " << t << " at "
                      << trash_bin_sequence_in_this_room[t] << "\ttraveling distance: "
                      << (trash_bin_dist1 + trash_bin_dist2) * evaluation_data.map_resolution_ << " m" << std::endl;
            screenoutput << "  room: " << room_index << "\ttrash bin: " << t << " at "
                         << trash_bin_sequence_in_this_room[t] << "\ttraveling distance: "
                         << (trash_bin_dist1 + trash_bin_dist2) * evaluation_data.map_resolution_ << " m" << std::endl;
        ...
        screenoutput << " cleaned all rooms and trash bins in current clique";
      }
      ...
      path_length_trolley += planner.planPath(evaluation_data.floor_plan_, downsampled_map, trolley_position,
                                              robot_start_position, evaluation_data.map_downsampling_factor_, 0.,
                                              evaluation_data.map_resolution_, 1, &draw_path_map);
      ...
      output << "robot_speed_without_trolley"
             << "\t"
             << "robot_speed_with_trolley"
             << "\t"
             << "time_for_trashbin_manipulation"
             << "\t"
             << "time_for_trolley_manipulation"
             << "\t"
             << "number_of_trash_bins"
             << "\t"
             << "number_trolley_movements"
             << "\t"
             << "path_length_robot_in_meter"
             << "\t"
             << "path_length_trolley_in_meter"
             << "\t"
             << "path_length_trash_bins_in_meter\t"
             << "pathlength [m]"
             << "\t"
             << "cleaning time [s]"
             << "\t"
             << "calculation time segmentation [s]"
             << "\t"
             << "calculation time sequencer [s]"
             << "\t" << std::endl;
      output << robot_speed_without_trolley << "\t" << robot_speed_with_trolley << "\t"
             << time_for_trashbin_manipulation << "\t" << time_for_trolley_manipulation << "\t"
             << evaluation_data.trash_bin_locations_.size() << "\t" << (result_seq->checkpoints.size() + 1) << "\t"
             << path_length_robot_in_meter << "\t" << path_length_trolley_in_meter << "\t"
             << path_length_trash_bins_in_meter << "\t" << path_length_total_in_meter << "\t" << time << "\t"
             << segmentation_time << "\t" << sequence_time;
  ...
  bool evaluateAllConfigsVer1(const std::vector<EvaluationConfig>& evaluation_configuration_vector,
                              const EvaluationData& evaluation_data, const std::string& data_storage_path)
  {
    // go through each configuration for the given map
    for (size_t config = 0; config < evaluation_configuration_vector.size(); ++config)
    {
      ...
      int switching_trolley_handling =
          0;  // int that shows how often the trolley has been handled during the switching procedure
              //			const double max_clique_path_length_in_pixel = evaluation_configuration.max_clique_path_length_ /
              //evaluation_data.map_resolution_;
      cv::Point robot_position = robot_start_position;
      cv::Point trolley_position(
      ...
      for (size_t clique_index = 0; clique_index < result_seq->checkpoints.size(); ++clique_index)
      {
        ...
        if (current_emptied_trashbins == evaluation_configuration_vector[config].trashbins_per_trolley_)
        {
          ...
          path_length_trolley += planner.planPath(
              evaluation_data.floor_plan_, downsampled_map, trolley_position, evaluation_data.central_trolley_park_,
              evaluation_data.map_downsampling_factor_, 0., evaluation_data.map_resolution_, 1, &draw_path_map);
          // ii) from central trolley park to new trolley position
          path_length_trolley +=
              planner.planPath(evaluation_data.floor_plan_, downsampled_map, evaluation_data.central_trolley_park_,
                               trolley_goal_position, evaluation_data.map_downsampling_factor_, 0.,
                               evaluation_data.map_resolution_, 1, &draw_path_map);
        ...
        for (size_t room = 0; room < result_seq->checkpoints[clique_index].room_indices.size(); ++room)
        {
          ...
          if (current_emptied_trashbins == evaluation_configuration_vector[config].trashbins_per_trolley_)
          {
            ...
            path_length_trolley +=
                2.0 *
                planner.planPath(evaluation_data.floor_plan_, downsampled_map, trolley_position,
                                 evaluation_data.central_trolley_park_, evaluation_data.map_downsampling_factor_, 0.,
                                 evaluation_data.map_resolution_, 1, &draw_path_map);
          ...
          cv::circle(draw_path_map, robot_position, 3, CV_RGB(0, 255, 0), -1);
          // clear all trash bins: go to trash bin, go back to trolley to empty trash and then drive back to trash bin
          std::cout << " arrived in room " << current_roomcenter << "\n starting to plan the trash bins" << std::endl;
          screenoutput << " arrived in room " << current_roomcenter << "\n starting to clean the trash bins"
                       << std::endl;
          ipa_building_msgs::FindRoomSequenceWithCheckpointsResultConstPtr result_trash_bin_seq;
          std::vector<cv::Point> trash_bin_sequence_in_this_room;
          if (room_trash_bins[room_index].size() > 1)
          {
            bool result_trash =
                computeTrashBinSequence(evaluation_data, evaluation_configuration_vector[config],
                                        room_trash_bins[room_index], robot_position, result_trash_bin_seq);
          ...
          std::cout << "starting to clean the trashbins. Size of sequence: " << trash_bin_sequence_in_this_room.size()
                    << std::endl;
          for (size_t t = 0; t < trash_bin_sequence_in_this_room.size(); ++t)
          {
            // check if the trolley needs to be changed
  ...
  bool evaluateAllConfigsVer2(const std::vector<EvaluationConfig>& evaluation_configuration_vector,
                              const EvaluationData& evaluation_data, const std::string& data_storage_path)
  {
    // go through each configuration for the given map
    for (size_t config = 0; config < evaluation_configuration_vector.size(); ++config)
    {
      ...
      DynamicReconfigureClient drc(node_handle_, "/room_sequence_planning/room_sequence_planning_server/set_parameters",
                                   "/room_sequence_planning/room_sequence_planning_server/parameter_updates");
      ...
      ipa_building_msgs::FindRoomSequenceWithCheckpointsResultConstPtr result_seq;
      clock_gettime(CLOCK_MONOTONIC, &t2);  // set time stamp before the sequence planning
      if (computeRoomSequence(evaluation_data, evaluation_configuration_vector[config],
                              evaluation_data.trash_bin_locations_, result_seq, t2) == false)
      {
        std::cout << "++++++++++ computeRoomSequence failed ++++++++++++" << std::endl;
        return false;
      }
      ...
      for (size_t checkpoint = 0; checkpoint < result_seq->checkpoints.size(); ++checkpoint)
      {
        ...
        for (size_t trash = 0; trash < current_trashbins.size(); ++trash)
        {
          int current_room_index =
              segmented_map.at<int>(evaluation_data.trash_bin_locations_[current_trashbins[trash]]);
      ...
      for (size_t room = 0; room < room_centers.size(); ++room)
      {
        ...
        if (contains(assigned_rooms, current_room_index) == false)
        {
          ...
          for (size_t checkpoint = 0; checkpoint < result_seq->checkpoints.size(); ++checkpoint)
          {
            cv::Point trolley_position = cv::Point(result_seq->checkpoints[checkpoint].checkpoint_position_in_pixel.x,
                                                   result_seq->checkpoints[checkpoint].checkpoint_position_in_pixel.y);
            double current_distance =
                planner.planPath(evaluation_data.floor_plan_, downsampled_map, room_centers[room], trolley_position,
                                 evaluation_data.map_downsampling_factor_, 0., evaluation_data.map_resolution_);
            ;
      ...
      for (size_t clique_index = 0; clique_index < result_seq->checkpoints.size(); ++clique_index)
      {
        ...
        trolley_position = cv::Point(result_seq->checkpoints[clique_index].checkpoint_position_in_pixel.x,
                                     result_seq->checkpoints[clique_index].checkpoint_position_in_pixel.y);
        ...
        if (computeTrashBinSequence(evaluation_data, evaluation_configuration_vector[config],
                                    rooms_for_trolley_position[clique_index], robot_position, result_room_seq) == true)
        {
          for (size_t cc = 0; cc < result_room_seq->checkpoints.size(); ++cc)
            for (size_t tt = 0; tt < result_room_seq->checkpoints[cc].room_indices.size(); ++tt)
              current_room_order.push_back(
                  rooms_for_trolley_position[clique_index][result_room_seq->checkpoints[cc].room_indices[tt]]);
        }
        ...
        for (size_t room = 0; room < current_room_order.size(); ++room)
        {
          ...
          {
            cv::Point trashbin =
                evaluation_data.trash_bin_locations_[result_seq->checkpoints[clique_index].room_indices[trash]];
            if (room_index == segmented_map.at<int>(trashbin))
            {
              ...
              path_length_robot += planner.planPath(evaluation_data.floor_plan_, downsampled_map, robot_position,
                                                    trashbin, evaluation_data.map_downsampling_factor_, 0.,
                                                    evaluation_data.map_resolution_, 1, &draw_path_map);
              path_length_trash_bins += 2.0 *
                                        planner.planPath(evaluation_data.floor_plan_, downsampled_map, trashbin,
                                                         trolley_position, evaluation_data.map_downsampling_factor_, 0.,
                                                         evaluation_data.map_resolution_, 1, &draw_path_map);
      ...
      output << robot_speed_without_trolley << "\t" << robot_speed_with_trolley << "\t"
             << time_for_trashbin_manipulation << "\t" << time_for_trolley_manipulation << "\t"
             << evaluation_data.trash_bin_locations_.size() << "\t" << 0 << "\t" << path_length_robot_in_meter << "\t"
             << path_length_trolley_in_meter << "\t" << path_length_trash_bins_in_meter << "\t"
             << path_length_total_in_meter << "\t" << time << "\t" << segmentation_time << "\t" << sequence_time;
      output << "\n\n\n" << screenoutput.str() << std::endl;
      ...
      {
        cv::Point trolley_placing_position = cv::Point(result_seq->checkpoints[pos].checkpoint_position_in_pixel.x,
                                                       result_seq->checkpoints[pos].checkpoint_position_in_pixel.y);
        travel_distance_human +=
            2.0 *
            planner.planPath(evaluation_data.floor_plan_, downsampled_map, trolley_placing_position,
                             robot_start_position, evaluation_data.map_downsampling_factor_, 0.,
                             evaluation_data.map_resolution_, 1);
      }
      travel_distance_human = travel_distance_human * evaluation_data.map_resolution_;
      std::string storage_path_human =
          data_storage_path + "human_way/" + upper_folder_name +
          evaluation_configuration_vector[config].generateLowerConfigurationFolderString() + "/";
                        ...
                        ipa_building_msgs::MapSegmentationResultConstPtr& result_seg, struct timespec& t0)
  {
    ...
    do
    {
      ...
      actionlib::SimpleActionClient<ipa_building_msgs::MapSegmentationAction> ac_seg(
          "/room_segmentation/room_segmentation_server", true);
      ...
      DynamicReconfigureClient drc(node_handle_, "/room_segmentation/room_segmentation_server/set_parameters",
                                   "/room_segmentation/room_segmentation_server/parameter_updates");
  ...
  bool computeTrashBinSequence(const EvaluationData& evaluation_data, const EvaluationConfig& evaluation_configuration,
                               ...
                               const cv::Point& robot_start_position,
                               ipa_building_msgs::FindRoomSequenceWithCheckpointsResultConstPtr& result_seq)
  {
    bool planned = false;
    ...
    actionlib::SimpleActionClient<ipa_building_msgs::FindRoomSequenceWithCheckpointsAction> ac_seq(
        "/room_sequence_planning/room_sequence_planning_server", true);
    ...
    goal_seq.robot_start_coordinate.position.x =
        robot_start_position.x * evaluation_data.map_resolution_ + evaluation_data.map_origin_.position.x;
    goal_seq.robot_start_coordinate.position.y =
        robot_start_position.y * evaluation_data.map_resolution_ + evaluation_data.map_origin_.position.y;
