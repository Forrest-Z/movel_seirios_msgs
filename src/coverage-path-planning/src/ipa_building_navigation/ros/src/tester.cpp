...
int main(int argc, char** argv)
{
  ...
  std::vector<int> nearest_neighbor_order =
      genTSPsolver.solveGeneticTSP(map, n, centers, downfactor, robot_radius_test, map_resolution_factor, 0);
  ...
  nearest_neighbor_order = genTSPsolver.solveGeneticTSP(map, n, centers, downfactor, robot_radius_test,
                                                        map_resolution_factor, 0, distancematrix);
  ...
  for (int i = 0; i < centers.size(); i++)
  {
    ...
    for (int p = 0; p < centers.size(); p++)
    {
      if (p != i)
      {
        ...
        {
          double length =
              planner.planPath(map, center, centers[p], downfactor, robot_radius_test, map_resolution_factor);
  ...
  std::vector<std::vector<int> > new_groups =
      setsolver.solveSetCover(map, n, centers, downfactor, robot_radius_test, map_resolution_factor, 150.0);

  ROS_INFO("Starting to find the trolley positions.");

  std::vector<cv::Point> trolley_positions =
      tolley_finder.findTrolleyPositions(map, groups, centers, downfactor, robot_radius_test, map_resolution_factor);
  std::vector<cv::Point> new_trolleys = tolley_finder.findTrolleyPositions(map, new_groups, centers, downfactor,
                                                                           robot_radius_test, map_resolution_factor);
