...
//
// This class provides a maximal clique-finder for a given Graph that finds all maximal cliques in this. A maximal
// clique
// is a subgraph in the given Graph, in which all Nodes are connected to each other and cannot be enlarged by adding
// other
// Nodes ( https://en.wikipedia.org/wiki/Maximum_clique ). It uses the c++ Boost library by using the implementen
// Bron-Kerbosch
...
typename graph_traits<Graph>::vertex_descriptor cliqueFinder::addNamedVertex(Graph& g, NameMap nm, const string& name,
                                                                             VertexMap& vm)
{
...
// This class is the Visitor for all nodes in the graph, which is used by boost::bron_kerbosch. the function clique()
// gets
...
// This function uses the previously described functions and finds all maximal cliques in a given graph. The maxval
// parameter
