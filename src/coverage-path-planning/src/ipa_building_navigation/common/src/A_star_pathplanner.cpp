...
// the
// map gets reduced and calculation time gets better. If it is set to 1 the map will have original size, if it is 0 the
// algorithm
// won't work, so make sure to not set it to 0. The algorithm also needs the Robot radius [m] and the map resolution
// [m²/pixel] to
                              ...
                              cv::Mat* draw_path_map, std::vector<cv::Point>* route)
{
  ...
  //	cv::circle(debug, start_point, 2, cv::Scalar(127), cv::FILLED);
  //	cv::circle(debug, end_point, 2, cv::Scalar(127), cv::FILLED);
