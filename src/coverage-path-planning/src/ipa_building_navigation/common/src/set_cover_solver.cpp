...
///and
////the columns to which node to go. If the path between nodes doesn't exist or the node to go to is the same as the one
///to
...
//					double length = one_by_downsampling_factor * pathplanner_.planPath(downsampled_map, current_center,
//neighbor, 1., 0., map_resolution);
...
std::vector<std::vector<int> > SetCoverSolver::mergeGroups(const std::vector<std::vector<int> >& found_groups)
{
  ...
  {
    if (!contains(done_groups, current_group))  // If the group is in the done-vector don't look at it, because it has
...
// This functions solves the set-cover Problem ( https://en.wikipedia.org/wiki/Set_cover_problem#Greedy_algorithm )
// using
// the greedy-search algorithm. It chooses the clique that has the most uncovered nodes in it first. Then it uses the
// merge-function
// above to merge groups that have at least one node in common together. The vector stores the indexes of the nodes,
// which
// are the same as the ones from the clique-solver and also the distance-matrix. The variable
// max_number_of_clique_members
...
std::vector<std::vector<int> > SetCoverSolver::solveSetCover(std::vector<std::vector<int> >& given_cliques,
                                                             ...
                                                             const int max_number_of_clique_members,
                                                             const cv::Mat& distance_matrix)
{
  ...
  // Put the nodes in a open-nodes vector. The nodes are named after their position in the room-centers-vector and so
  // every
  ...
  do
  {
    ...
    // check if a allowed clique could be found, if not split the biggest clique until it consists of cliques that are
    // of the
    // allowed size
    if (best_clique == -1)
    {
    ...
    // remove nodes of best clique from all cliques (this is okay because if you remove a node from a clique it stays a
    // clique, it only isn't a maximal clique anymore)
    for (size_t clique = 0; clique < cliques_for_covering.size(); clique++)
    {
      for (int node = 0; node < cliques_for_covering[best_clique].size(); node++)
      {
        if (clique != best_clique)
          cliques_for_covering[clique].erase(std::remove(cliques_for_covering[clique].begin(),
                                                         cliques_for_covering[clique].end(),
                                                         cliques_for_covering[best_clique][node]),
                                             cliques_for_covering[clique].end());
      }
    }
    for (int node = 0; node < cliques_for_covering[best_clique].size(); node++)
    {
      open_nodes.erase(std::remove(open_nodes.begin(), open_nodes.end(), cliques_for_covering[best_clique][node]),
                       open_nodes.end());
    }
...
std::vector<std::vector<int> > SetCoverSolver::solveSetCover(const cv::Mat& distance_matrix,
                                                             const std::vector<cv::Point>& points,
                                                             const int number_of_nodes, double maximal_pathlength,
                                                             const int max_number_of_clique_members)
{
  // get all maximal cliques for this graph
  std::vector<std::vector<int> > maximal_cliques =
      maximal_clique_finder.getCliques(distance_matrix, maximal_pathlength);
...
std::vector<std::vector<int> > SetCoverSolver::solveSetCover(
    const cv::Mat& original_map, const std::vector<cv::Point>& points, double downsampling_factor, double robot_radius,
    double map_resolution, double maximal_pathlength, const int max_number_of_clique_members, cv::Mat* distance_matrix)
{
  ...
  return (
      solveSetCover(distance_matrix_ref, points, (int)points.size(), maximal_pathlength, max_number_of_clique_members));
}
