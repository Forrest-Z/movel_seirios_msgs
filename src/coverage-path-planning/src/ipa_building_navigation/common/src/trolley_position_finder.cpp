...
//			 has only two members the algorithm chooses the candidate as trolley position that is the middlest between
//these.
cv::Point TrolleyPositionFinder::findOneTrolleyPosition(const std::vector<cv::Point> group_points,
                                                        const cv::Mat& original_map, const double downsampling_factor,
                                                        const double robot_radius, const double map_resolution)
{
  double largening_of_bounding_box = 5;  // Variable to expand the bounding box of the roomcenters a little bit. This is
                                         // done to make sure the best trolley position is found if it is a little bit
                                         // outside this bounding box.
  double max_x_value = group_points[0].x;  // max/min values of the Points that get the bounding box. Initialized with
                                           // the coordinates of the first Point of the group.
  ...
  // expand the bounding box sligthly by the defined factor (check if the values aren't out of the map boundaries before
  // doing this)
  ...
  {
    max_x_cell = min_x_value + cell_side_length;  // reset the x-values for the cell to start from beginning when a new
    ...
    do  // go from x_min to x_max
    {
      ...
      for (int y = min_y_cell; y < max_y_cell; y++)
      {
        ...
        {
          if (distance_map.at<unsigned char>(best_y, best_x) < distance_map.at<unsigned char>(y, x) &&
              eroded_map.at<unsigned char>(y, x) != 0)
          {
            best_x = x;
            best_y = y;
          }
  ...
  for (size_t candidate = 0; candidate < trolley_position_candidates.size(); candidate++)
  {
    ...
    for (int room_center = 0; room_center < group_points.size(); room_center++)
    {
      ...
      double center_pathlength =
          one_by_downsampling_factor *
          path_planner_.planPath(downsampled_map, start_point, end_point, 1., 0., map_resolution);
      pathlengths.push_back(center_pathlength);
      // add the pathlenght to the total pathlength
      current_pathlength += center_pathlength;
    }
    // check for the best position that has the shortest pathlength to all centers. Adding a little bit to the
    // best_distances
    ...
    if (group_points.size() == 2)
    {
      ...
      if (current_pathlength <= (best_pathlength + 0.05) &&
          current_point_distance <= (best_pathlength_point_distance + 0.05) &&
          downsampled_map.at<unsigned char>(downsampling_factor *
                                            trolley_position_candidates[best_trolley_candidate]) != 0)
      {
...
// described functions.
std::vector<cv::Point> TrolleyPositionFinder::findTrolleyPositions(const cv::Mat& original_map,
                                                                   const std::vector<std::vector<int> >& found_groups,
                                                                   const std::vector<cv::Point>& room_centers,
                                                                   const double downsampling_factor,
