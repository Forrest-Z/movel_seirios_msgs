...
// Default constructor
GeneticTSPSolver::GeneticTSPSolver() : abort_computation_(false)
{
}
...
// random
// nodes can be switched, or a random intervall of nodes can be inverted. Only the first and last Node can't be changed,
// because
...
std::vector<int> GeneticTSPSolver::mutatePath(const std::vector<int>& parent_path)
{
  ...
  else if (what_to_change == 1)  // random intervall-inverting (always from middle node on)
  {
    ...
    do
    {
      ...
      if (node_one >
          node_two)  // switch variables, if the node_one is bigger than the node_two (easier to work with here)
      {
...
std::vector<int> GeneticTSPSolver::solveGeneticTSP(const cv::Mat& path_length_Matrix, const int start_Node)
{
  ...
  if (path_length_Matrix.rows >
      2)  // check if graph has at least three members, if not the algorithm won't work properly
  {
    ...
    do
    {
      ...
      if (number_of_generations >=
          2300)  // when a specified amount of steps have been done the algorithm checks if the last paths didn't change
      {
        if (changed_path)
        {
