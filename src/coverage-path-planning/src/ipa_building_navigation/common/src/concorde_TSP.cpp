...
// This function generates a file with the current TSP in TSPlib format. This is necessary because concorde needs this
// file
// as input to solve the TSP. See http://comopt.ifi.uni-heidelberg.de/software/TSPLIB95/ for documentation.
void ConcordeTSPSolver::writeToFile(const cv::Mat& pathlength_matrix, const std::string& tsp_lib_filename,
                                    const std::string& tsp_order_filename)
{
  const std::string path_for_saving_file =
      tsp_lib_filename;  // ros::package::getPath("libconcorde_tsp_solver") + "/common/files/TSPlib_file.txt";
  ...
  if (saving_file.is_open())
  {
    ...
    // specify name of the Problem, Type (TSP = symmetrical TSP) and add a comment to the file. Name and Type are
    // necessary, comment is for better understanding when you open the file.
    saving_file << "NAME: ipa-building-navigation_" << tsp_lib_filename << std::endl
                << "TYPE: TSP" << std::endl
                << "COMMENT: This is the TSPlib file for using concorde. See "
                   "http://comopt.ifi.uni-heidelberg.de/software/TSPLIB95/ for documentation."
                << std::endl;
    saving_file << "DIMENSION: " << pathlength_matrix.cols
                << std::endl;  // Shows the Dimension of the problem --> the number of nodes (Necessary).
...
// This function opens the file which saves the output from the concorde solver and reads the saved order. The names of
// the
// nodes in the graph are stored as positions in the distance matrix in this case. The first integer in the file is the
// number
...
{
  std::string path_for_order_file = tsp_order_filename;  // ros::package::getPath("libconcorde_tsp_solver") +
                                                         // "/common/files/TSP_order.txt"; //get path to file
...
// First you have to build the Solver in any possible way and if you don't make a ros-package out of it you have to
// change
...
std::vector<int> ConcordeTSPSolver::solveConcordeTSP(const cv::Mat& path_length_matrix, const int start_Node)
{
  ...
  if (path_length_matrix.rows > 2)  // check if the TSP has at least 3 nodes
  {
    ...
    while (bin_folder.length() == 0)
    {
      ...
      {
        std::cout << "ConcordeTSPSolver::solveConcordeTSP: ERROR: ['rospack libs-only-L libconcorde_tsp_solver > '"
                  << temp_file << "] failed. Trying again." << std::endl;
      }
    }
    std::string cmd = bin_folder + "/libconcorde_tsp_solver/concorde -o " + "$HOME/.ros/" + tsp_order_filename +
                      " $HOME/.ros/" + tsp_lib_filename;
  ...
  {
    std::cout << "ConcordeTSPSolver::solveConcordeTSP: Warning: Optimized order invalid, taking standard order 1, 2, "
                 "3, ..."
              << std::endl;
...
std::vector<int> ConcordeTSPSolver::solveConcordeTSPClean(const cv::Mat& original_map,
                                                          ...
                                                          double map_resolution, const int start_node)
{
  ...
  return solveConcordeTSPWithCleanedDistanceMatrix(distance_matrix_cleaned, cleaned_index_to_original_index_mapping,
                                                   new_start_node);
}

// compute TSP with pre-computed cleaned distance matrix (does not contain any infinity paths)
std::vector<int> ConcordeTSPSolver::solveConcordeTSPWithCleanedDistanceMatrix(
