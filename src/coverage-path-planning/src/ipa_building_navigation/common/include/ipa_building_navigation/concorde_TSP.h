...
// This class applies an object to solve a given TSP problem using the concorde TSP solver. This solver can be
// downloaded at:
...
// If you have build the solver navigate to the./TSP folder and type " ./concorde -h " to see how to use this solver.
// This class
// uses the concorde solver by using a systemcall.
// Make sure you have a "/common/files/TSP_order.txt" and a "/common/files/TSPlib_file.txt" file. These files are used
// to tell
...
protected:
  ...
  void writeToFile(const cv::Mat& pathlength_matrix, const std::string& tsp_lib_filename,
...
public:
  ...
  // Functions to solve the TSP. It needs a distance matrix, that shows the pathlengths between two nodes of the
  // problem.
  ...
  std::vector<int> solveConcordeTSP(const cv::Mat& original_map, const std::vector<cv::Point>& points,
                                    double downsampling_factor, double robot_radius, double map_resolution,
  ...
  std::vector<int> solveConcordeTSPWithCleanedDistanceMatrix(
