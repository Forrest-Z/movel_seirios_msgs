...
// This object provides an algorithm to search for the best trolley position for a group of points. A trolley position
// is
// the Point where the trolley of a robot should be placed during the cleaning of the group. This is done by searching
// in the
// bounding box of these points for the point which minimizes the pathlength to every group member. If the goup has only
// two
// members the algorithm chooses a Point on the optimal path between these two Points that is in the middlest of this
// path.
...
//		5. The Radius of the robot and the map resolution to make sure the A_star pathplanner stays in enough distance to
//the
...
public:
  ...
  std::vector<cv::Point> findTrolleyPositions(const cv::Mat& original_map,
