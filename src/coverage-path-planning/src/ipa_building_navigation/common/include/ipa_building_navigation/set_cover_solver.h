...
// This algorithm provides a class to solve the set-cover problem for given cliques. This is done by using the
// greedy-search
// algorithm, which takes the clique with most unvisited nodes before the other nodes and removes the nodes in it from
// the
// unvisited. It repeats this step until no more node hasn't been visited. It then merges cliques together that have at
// least
// one node in common.
//
//!!!!!!!!!!!!!!!!Important!!!!!!!!!!!!!!!!!
// Make sure that the cliques cover all nodes in the graph or else this algorithm runs into an endless loop. For best
// results
...
protected:
  ...
  // This object finds all maximal cliques in the given Graph. It needs a symmetrical distance matrix shwoing the
  // pathlength
  // from one node to another and a miximal pathlength to cut edges that are larger than this value. See
  // maximal_clique_finder.h
  ...
  //	void constructDistanceMatrix(cv::Mat& distance_matrix, const cv::Mat& original_map, const std::vector<cv::Point>&
  //points,
...
public:
  ...
  std::vector<std::vector<int> > solveSetCover(const cv::Mat& original_map, const std::vector<cv::Point>& points,
                                               double downsampling_factor, double robot_radius, double map_resolution,
                                               double maximal_pathlength, const int max_number_of_clique_members,
                                               cv::Mat* distance_matrix = 0);
};
