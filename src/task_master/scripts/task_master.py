...
class TaskMaster:
    ...
    def __init__(self):
        ...
        try:
            ...
            loop.create_task(
                self.comm.subscribe(
                    routing_key = self.robot_cancel_key,
                    async_msg_proc_func = self.process_cancel_msg,
                    reply_routing_key = self.robot_cancel_reply_key,
                    reply_prep = self.msg_to_json
                )
            )
            loop.create_task(
                self.comm.subscribe(
                    routing_key = self.robot_launch_key,
                    async_msg_proc_func = self.process_task_msg,
                    reply_routing_key = self.robot_launch_reply_key,
                    reply_prep = self.msg_to_json
                )
            )
            loop.run_forever()
        ...
        try:
            ...
            if msg['Timeout'] is not None and \
                ...
                msg['Launchfile'] is None:
                    rospy.loginfo("Starting a ROS node with params")
                    name, success, statement = await self.start_ros_node(
                        pkg = msg['Pkg'],
                ...
                msg['Args'] is None and \
                msg['Launchfile'] is None:
                    rospy.loginfo("Starting a ROS node without params")
                    name, success, statement = await self.start_ros_node(
                        pkg = msg['Pkg'],
                        executable = msg['Executable'],
                        timeout = msg['Timeout']
                ...
                msg['Launchfile'] is None:
                    rospy.loginfo("Starting an executable")
                    name, success, statement = await self.start_executable(
                        executable = msg['Executable'],
                        args = msg['Args'],
                        timeout = msg['Timeout']
                    )
                ...
                msg['Args'] is None and \
                msg['Launchfile'] is None:
                    rospy.loginfo("Starting an exeuctable without args")
                    name, success, statement = await self.start_executable(
                        executable = msg['Executable'],
                        timeout = msg['Timeout']
                    )
            ...
            for k, v in self.running_launches.items():
                ...
                if clean == k:
                    ...
                    try:
                        ...
                        result = True
                        msg = "Cleaned"
                        rospy.loginfo("Deleting %s from running_launchfiles", k)
                        cleaned.append(k)
                    except Exception as err:
            ...
            for k, v in self.running_nodes.items():
                ...
                if clean == k:
                    ...
                    try:
                        ...
                        result = True
                        msg = "Cleaned"
                        rospy.loginfo("Deleting %s from running_nodes", k)
                        cleaned.append(k)
    ...
    async def async_stop(self):
        ...
        """
        rospy.loginfo("Cleaning up all the nodes launched")
    ...
    async def start_launchfile(self, launchfile, timeout, pkg=None):
        ...
        try:
            ...
            rospy.loginfo("Starting ROS Launchfile: %s", name)
            pid_list = []
    ...
    async def start_ros_node(self, pkg, executable, timeout, params=None):
        ...
        try:
            ...
            rospy.loginfo("Starting ROS Node: %s", name)
    ...
    async def start_executable(self, executable, timeout, args=None):
        ...
        try:
            ...
            rospy.loginfo("Starting executable: %s", name)
...
def main() -> None:
    rospy.init_node("task_master", disable_signals=False)
    tm = TaskMaster()
    rospy.spin()
