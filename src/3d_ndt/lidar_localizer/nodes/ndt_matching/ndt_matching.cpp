...
static std::string odom_frame_id_ = "/odom";
static std::string base_frame_id_ = "/base_link";
static std::string global_frame_id_ = "/map";
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  // ROS_INFO("IM IN POINTS_CALLBACK");
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    tf::Quaternion predict_q, ndt_q, localizer_q;
    ...
    */
    ...
    tf2::Transform delta = transform * baseInMapTf2;
    m_offsetTf = delta * m_offsetTf;


    geometry_msgs::TransformStamped transformStamped;
    transformStamped.header.stamp = current_scan_time;
    transformStamped.header.frame_id = global_frame_id_;
    transformStamped.child_frame_id = odom_frame_id_;
    tf2::convert(m_offsetTf, transformStamped.transform);

    // transformStamped.transform.translation.x = current_pose.x;
    // transformStamped.transform.translation.y = current_pose.y;
    // transformStamped.transform.translation.z = current_pose.z;
    // // tf2::Quaternion current_q;
    // current_q.setRPY(current_pose.roll, current_pose.pitch, current_pose.yaw);
    // transformStamped.transform.rotation.x = current_q.x();
    // transformStamped.transform.rotation.y = current_q.y();
    // transformStamped.transform.rotation.z = current_q.z();
    // transformStamped.transform.rotation.w = current_q.w();
...
int main(int argc, char** argv)
{
  ...
  angle.setRPY(0.0, 0.0, 0.0);
  m_offsetTf = tf2::Transform(angle, tf2::Vector3(0.0, 0.0, 0.0));
