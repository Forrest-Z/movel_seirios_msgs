...
////////////////////////////////////////////////////////////////////
ChaspLegacy::ChaspLegacy(const Chasp& other) : ChaspBase(other)
{
  if (!other.hasLegacy())
    DIAG_VERIFY(release());
}
...
//! Copy constructor intializing object with other's private key.
////////////////////////////////////////////////////////////////////
ChaspLegacy::ChaspLegacy(const ChaspLegacy& other) : ChaspBase(other)
{
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspLegacy::decrypt(unsigned char* pData, hasp_size_t ulSize) const
{
...
haspStatus ChaspLegacy::decrypt(std::string& data) const
{
  ...
  haspStatus nStatus = decrypt(&vector[0], static_cast<hasp_size_t>(vector.size()));
  if (HASP_STATUS_OK != nStatus)
    return nStatus;

  data.resize(0);
  for (std::vector<unsigned char>::const_iterator it = vector.begin(); (vector.end() != it) && (0 != *it); it++)
    data += *it;
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspLegacy::encrypt(unsigned char* pData, hasp_size_t ulSize) const
{
  HASP_PROLOGUE(m_handle);
...
haspStatus ChaspLegacy::encrypt(std::string& data) const
{
  ...
  haspStatus nStatus = encrypt(&vector[0], static_cast<hasp_size_t>(vector.size()));
  if (HASP_STATUS_OK != nStatus)
    return nStatus;
