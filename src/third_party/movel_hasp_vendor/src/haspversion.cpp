...
ChaspVersion::ChaspVersion() : m_nMajorVersion(0), m_nMinorVersion(0), m_nServerBuild(0), m_nBuildNumber(0)
{
}

ChaspVersion::ChaspVersion(const ChaspVersion& version)
  : m_nMajorVersion(0), m_nMinorVersion(0), m_nServerBuild(0), m_nBuildNumber(0)
{
  *this = version;
}

ChaspVersion::ChaspVersion(unsigned int nMajorVersion, unsigned int nMinorVersion, unsigned int nServerBuild,
...
bool ChaspVersion::operator==(const ChaspVersion& version) const
{
  return (this == &version) ||
         ((majorVersion() == version.majorVersion()) && (minorVersion() == version.minorVersion()) &&
          (serverBuild() == version.serverBuild()) && (buildNumber() == version.buildNumber()));
}
