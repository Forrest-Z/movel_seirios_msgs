...
ErrorPrinter::ErrorPrinter()
{
  ...
  errorMap.insert(StatusMap::value_type(HASP_REMOTE_COMM_ERR, "Communication error between local and remote License "
                                                              "Manager"));
  ...
  errorMap.insert(StatusMap::value_type(HASP_HARDWARE_MODIFIED, "HASP update incompatible with this hardware; HASP key "
                                                                "is locked to other hardware"));
  ...
  errorMap.insert(StatusMap::value_type(HASP_UNKNOWN_RECIPIENT, "Unknown Recipient; update can only be applied to the "
                                                                "Recipient specified in hasp_detach(), and not to this "
                                                                "computer"));
  ...
  errorMap.insert(StatusMap::value_type(HASP_UPDATE_ALREADY_ADDED, "Specified V2C update already installed in the "
                                                                   "LLM"));
  ...
  errorMap.insert(StatusMap::value_type(HASP_EXTENSION_NOT_ALLOWED, "Extension of license not allowed as number of "
                                                                    "detached licenses is greater than current "
                                                                    "concurrency count"));
  errorMap.insert(StatusMap::value_type(HASP_DETACH_DISABLED, "Detach of license not allowed as product contains VM "
                                                              "disabled feature and host machine is a virtual "
                                                              "machine"));
  errorMap.insert(StatusMap::value_type(HASP_REHOST_DISABLED, "Rehost of license not allowed as container contains VM "
                                                              "disabled feature and host machine is a virtual "
                                                              "machine"));
  errorMap.insert(StatusMap::value_type(HASP_DETACHED_LICENSE_FOUND, "Format SL-AdminMode or migrate SL-Legacy to "
                                                                     "SL-AdminMode not allowed as container has "
                                                                     "detached license"));
  errorMap.insert(StatusMap::value_type(HASP_RECIPIENT_OLD_LM, "Recipient of the requested operation is older than "
                                                               "expected"));
  errorMap.insert(StatusMap::value_type(HASP_SECURE_STORE_ID_MISMATCH, "Secure storage ID mismatch"));
  errorMap.insert(StatusMap::value_type(HASP_DUPLICATE_HOSTNAME, "Duplicate Hostname found while key contains Hostname "
                                                                 "Fingerprinting"));
  errorMap.insert(StatusMap::value_type(HASP_MISSING_LM, "The Sentinel License Manager is required for this "
                                                         "operation"));
  errorMap.insert(StatusMap::value_type(HASP_FEATURE_INSUFFICIENT_EXECUTION_COUNT, "You are attempting to consume "
                                                                                   "multiple executions during log in "
                                                                                   "to a Feature"));
  errorMap.insert(StatusMap::value_type(HASP_INCOMPATIBLE_PLATFORM, "You are attempting to perform an operation not "
  ...
  errorMap.insert(StatusMap::value_type(HASP_KILLED_SESSION, "The session was killed due a network malfunction or "
                                                             "manually from ACC"));
  ...
  errorMap.insert(StatusMap::value_type(HASP_IDENTITY_DENIED, "The identity doesn't have enough permission for the "
                                                              "operation"));
  errorMap.insert(StatusMap::value_type(HASP_IDENTITY_SHARING_VIOLATION, "A session for this identity from a different "
                                                                         "machine already exists"));
  errorMap.insert(StatusMap::value_type(HASP_IDENTITY_TOO_MANY_MACHINES, "The maximum number of machines usable by the "
                                                                         "identity was reached"));

  errorMap.insert(StatusMap::value_type(HASP_NO_API_DYLIB, "A required API dynamic library was not found"));
  errorMap.insert(StatusMap::value_type(HASP_INV_API_DYLIB, "The found and assigned API dynamic library could not "
                                                            "verified"));
