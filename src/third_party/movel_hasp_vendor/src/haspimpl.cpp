...
ChaspImpl::ChaspImpl(hasp_feature_t feature, ChaspHandle& handle)
  : m_handle(0), m_feature(feature), m_ulState(stIdle), m_ulRefCount(0), m_ulAltered(0)
{
  addRef(handle);
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::decrypt(unsigned char* pData, hasp_size_t ulSize) const
{
  return isLoggedIn() ? hasp_decrypt(m_handle, pData, ulSize) : HASP_INV_HND;
}

////////////////////////////////////////////////////////////////////
//! Wrapper for the \a hasp_encrypt API function.
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::encrypt(unsigned char* pData, hasp_size_t ulSize) const
{
  return isLoggedIn() ? hasp_encrypt(m_handle, pData, ulSize) : HASP_INV_HND;
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::getFileSize(hasp_fileid_t fileId, hasp_size_t& ulSize) const
{
  return isLoggedIn() ? hasp_get_size(m_handle, fileId, &ulSize) : HASP_INV_HND;
}
...
{
  return isLoggedIn() ? hasp_get_rtc(m_handle, &time) : HASP_INV_HND;
}

////////////////////////////////////////////////////////////////////
//! Wrapper  for the \a hasp_get_sessioninfo API function.
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::getSessionInfo(const char* pszFormat, ChaspInfo& info) const
{
  if (NULL == pszFormat)
  ...
  return isLoggedIn() ? hasp_get_sessioninfo(m_handle, const_cast<char*>(pszFormat), &info.m_pszInfo) : HASP_INV_HND;
}

////////////////////////////////////////////////////////////////////
//! Wrapper  for the \a hasp_get_info API function.
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::getInfo(const char* pszQuery, const char* pszFormat, hasp_vendor_code_t vendorCode,
                              ChaspInfo& info)
{
  info.clear();

  return pszQuery && pszFormat ?
             hasp_get_info(const_cast<char*>(pszQuery), const_cast<char*>(pszFormat), vendorCode, &info.m_pszInfo) :
             HASP_INVALID_PARAMETER;
}

haspStatus ChaspImpl::transfer(const char* pszAction, const char* pszScope, hasp_vendor_code_t vendorCode,
                               const char* pszRecipient, ChaspInfo& v2c)
{
  v2c.clear();
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::getVersion(hasp_vendor_code_t vendorCode, ChaspVersion& version)
{
  ...
  haspStatus status = hasp_get_version(&nMajor, &nMinor, &nServer, &nBuildNo, vendorCode);

  version = HASP_SUCCEEDED(status) ? ChaspVersion(nMajor, nMinor, nServer, nBuildNo) : ChaspVersion();
...
bool ChaspImpl::isAltered(const ChaspHandle& handle) const
{
  ...
  return (0 >= m_ulRefCount) || (m_ulAltered != handle.m_ulAltered);
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::legacyDecrypt(unsigned char* pData, hasp_size_t ulSize) const
{
  return isLoggedIn() ? hasp_legacy_decrypt(m_handle, pData, ulSize) : HASP_INV_HND;
}

////////////////////////////////////////////////////////////////////
//! Wrapper for the \a hasp_legacy_encrypt API function.
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::legacyEncrypt(unsigned char* pData, hasp_size_t ulSize) const
{
  return isLoggedIn() ? hasp_legacy_encrypt(m_handle, pData, ulSize) : HASP_INV_HND;
}
...
{
  return isLoggedIn() ? hasp_legacy_set_rtc(m_handle, time) : HASP_INV_HND;
}
...
haspStatus ChaspImpl::login(hasp_vendor_code_t vendorCode, const char* pszScope)
{
  ...
  haspStatus status = pszScope ? hasp_login_scope(m_feature, const_cast<char*>(pszScope), vendorCode, &m_handle) :
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::readFile(hasp_fileid_t fileId, hasp_size_t ulOffset, unsigned char* pData,
                               hasp_size_t ulSize) const
{
  return isLoggedIn() ? hasp_read(m_handle, fileId, ulOffset, ulSize, pData) : HASP_INV_HND;
}
...
{
  return isLoggedIn() ? hasp_legacy_set_idletime(m_handle, nIdleTime) : HASP_INV_HND;
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::writeFile(hasp_fileid_t fileId, hasp_size_t ulOffset, const unsigned char* pData,
                                hasp_size_t ulSize) const
{
  if (!isLoggedIn())
    return HASP_INV_HND;

  return isLoggedIn() ? hasp_write(m_handle, fileId, ulOffset, ulSize, const_cast<unsigned char*>(pData)) :
                        HASP_INV_HND;
}
...
////////////////////////////////////////////////////////////////////
haspStatus ChaspImpl::update(const char* pszUpdate, ChaspInfo& acknowledge)
{
  if (NULL == pszUpdate)
    return HASP_INVALID_PARAMETER;
  ...
  return hasp_update(const_cast<char*>(pszUpdate), &acknowledge.m_pszInfo);
}
