...
////////////////////////////////////////////////////////////////////
ChaspFile::ChaspFile(hasp_fileid_t fileId, const Chasp& other) : ChaspBase(other)
{
  init(fileId);
}
...
//! Copy constructor.
////////////////////////////////////////////////////////////////////
ChaspFile::ChaspFile(const ChaspFile& other) : ChaspBase(other)
{
...
ChaspFile& ChaspFile::operator=(const ChaspFile& other)
{
  if (this != &other)
  {
    dynamic_cast<ChaspBase&>(*this) = dynamic_cast<const ChaspBase&>(other);
...
{
  return (NULL == szString) ? true : canWriteString(std::string(szString));
}
...
{
  return getFilePosFromString(std::string((NULL == szString) ? "" : szString));
}
...
{
  return canWriteString(string) ? static_cast<hasp_size_t>(string.length() + sizeof(unsigned char)) : 0;
}
...
//!                             setFilePos
////////////////////////////////////////////////////////////////////
haspStatus ChaspFile::read(unsigned char* pData, hasp_size_t ulSize) const
{
  HASP_PROLOGUE(m_handle);
  return HASP_KEYPTR->readFile(m_fileId, m_ulFilePos, pData, ulSize);
}
...
haspStatus ChaspFile::read(std::string& string) const
{
  ...
  if (0 < length)
  {
    ...
    status = read(&vector[0], static_cast<hasp_size_t>(vector.size() * sizeof(unsigned char)));
    if (!HASP_SUCCEEDED(status))
      return status;
...
//!                             setFilePos
////////////////////////////////////////////////////////////////////
haspStatus ChaspFile::write(const unsigned char* pData, hasp_size_t ulCount) const
{
  HASP_PROLOGUE(m_handle);
  return HASP_KEYPTR->writeFile(m_fileId, m_ulFilePos, pData, ulCount);
}
...
haspStatus ChaspFile::write(const std::string& string) const
{
  ...
  return write(&vector[0], static_cast<hasp_size_t>(vector.size()));
}
