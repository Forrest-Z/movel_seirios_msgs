...
//! \a ulFeature.
////////////////////////////////////////////////////////////////////
ChaspFeature::ChaspFeature(hasp_u32_t ulFeature) : m_ulFeature(ulFeature)
{
}
...
{
  return isProgNum() ? m_ulFeature & ~HASP_PROGNUM_OPT_MASK : m_ulFeature;
}
...
{
  return isProgNum() ? m_ulFeature & HASP_PROGNUM_OPT_MASK : optDefault;
}
...
bool ChaspFeature::hasOption(hasp_u32_t ulOption) const
{
  ...
  return (optDefault == ulOption) ? (optDefault == ulOptions) : (ulOption == (ulOptions & ulOption));
}
...
{
  return featureId() == static_cast<hasp_u32_t>(isProgNum() ? HASP_PROGNUM_DEFAULT_FID : HASP_DEFAULT_FID);
}
