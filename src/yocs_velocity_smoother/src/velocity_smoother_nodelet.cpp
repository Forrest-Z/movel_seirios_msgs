...
void VelocitySmoother::reconfigCB(yocs_velocity_smoother::paramsConfig &config, uint32_t level)
{
  ROS_INFO("Reconfigure request : %f %f %f %f %f",
           config.speed_lim_v, config.speed_lim_w, config.accel_lim_v, config.accel_lim_w, config.decel_lim_v, config.decel_lim_w);
  ...
  decel_lim_v  = config.decel_lim_v;
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  if ((nh.getParam("decel_lim_v", decel_lim_v) == false) ||
