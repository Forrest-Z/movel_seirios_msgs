...
private:
     ...
     */
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::AngularBoundsFilterConfig>> dyn_client_angle_; //shared ptr is used so that the client will not have the destructor called upon exiting it's current scope
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::AngularBoundsFilterInPlaceConfig>> dyn_client_angleinplace_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::BoxFilterConfig>> dyn_client_box_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::FootprintFilterConfig>> dyn_client_footprint_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::IntensityFilterConfig>> dyn_client_intensity_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::PolygonFilterConfig>> dyn_client_polygon_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::RangeFilterConfig>> dyn_client_range_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::ScanShadowsFilterConfig>> dyn_client_shadow_;
    std::shared_ptr<dynamic_reconfigure::Client<laser_filters::SpeckleFilterConfig>> dyn_client_speckle_;

    /**
     * Dynamic Reconfigure configurations
     */
    laser_filters::AngularBoundsFilterConfig anglecfg_;
    laser_filters::AngularBoundsFilterInPlaceConfig angleinplacecfg_;
    laser_filters::BoxFilterConfig boxcfg_;
    laser_filters::FootprintFilterConfig footprintcfg_;
    laser_filters::IntensityFilterConfig intensitycfg_;
    laser_filters::PolygonFilterConfig polygoncfg_;
    laser_filters::RangeFilterConfig rangecfg_;
    laser_filters::ScanShadowsFilterConfig shadowcfg_;
    laser_filters::SpeckleFilterConfig specklecfg_;
};
    ...
    if (ros::param::get(parampath, config_xml))
    {
        ...
        for (int i = 0; i < config["scan_filter_chain"].size(); i++) //iterate through all the filters in selected profile
        {
            ...
            if (filter_type == "laser_filters/LaserScanAngularBoundsFilter"){
                dyn_client_angle_.reset(new dynamic_reconfigure::Client<laser_filters::AngularBoundsFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanAngularBoundsFilterInPlace"){
                dyn_client_angleinplace_.reset(new dynamic_reconfigure::Client<laser_filters::AngularBoundsFilterInPlaceConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanBoxFilter"){
                dyn_client_box_.reset(new dynamic_reconfigure::Client<laser_filters::BoxFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanFootprintFilter"){
                dyn_client_footprint_.reset(new dynamic_reconfigure::Client<laser_filters::FootprintFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanIntensityFilter"){
                dyn_client_intensity_.reset(new dynamic_reconfigure::Client<laser_filters::IntensityFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanPolygonFilter"){
                dyn_client_polygon_.reset(new dynamic_reconfigure::Client<laser_filters::PolygonFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanRangeFilter"){
                dyn_client_range_.reset(new dynamic_reconfigure::Client<laser_filters::RangeFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/ScanShadowsFilter"){
                dyn_client_shadow_.reset(new dynamic_reconfigure::Client<laser_filters::ScanShadowsFilterConfig>(node_name_ + filter_name));
            }
            else if (filter_type == "laser_filters/LaserScanSpeckleFilter"){
                dyn_client_speckle_.reset(new dynamic_reconfigure::Client<laser_filters::SpeckleFilterConfig>(node_name_ + filter_name));
            }
        }
...
bool DynLaser::onProfileUpdate(laser_filters::ProfileUpdate::Request &req, laser_filters::ProfileUpdate::Response &res)
{
...
//     {
//         if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanAngularBoundsFilter")
//         {
...
//             dyn_client_angle_->setConfiguration(anglecfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanAngularBoundsFilterInPlace")
//         {
...
//             dyn_client_angleinplace_->setConfiguration(angleinplacecfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanBoxFilter")
//         {
...
//             dyn_client_box_->setConfiguration(boxcfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanFootprintFilter")
//         {
...
//             dyn_client_footprint_->setConfiguration(footprintcfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanIntensityFilter")
//         {
...
//             dyn_client_intensity_->setConfiguration(intensitycfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanPolygonFilter")
//         {
...
//             dyn_client_polygon_->setConfiguration(polygoncfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanRangeFilter")
//         {
...
//             dyn_client_range_->setConfiguration(rangecfg_);
//         }
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/ScanShadowsFilter")
//         {
...
//             dyn_client_shadow_->setConfiguration(shadowcfg_);
//         }        
//         else if ( config["scan_filter_chain"][i]["type"].as<std::string>() == "laser_filters/LaserScanSpeckleFilter")
//         {
