...
void HumanDetection::reconfigureCB(human_detection::human_detectionConfig &config, uint32_t level){
  /* Don't load initial config, since it will overwrite the rosparam settings */
  if (level == 0xFFFFFFFF) {
      return;
  }

  z_axis_min_ = config.z_axis_min;
  z_axis_max_ = config.z_axis_max ;
  detection_range_ = config.detection_range ;
  frames_tracked_ = config.frames_tracked ;
  ...
  cutoff_distance_  = config.cutoff_distance_from_robot ;
  voxel_grid_size_  = config.voxel_grid_leaf_size ;
  map_inflation_dist_  = config.map_inflation_dist ;
  max_width_length_diff_  = config.max_width_length_diff ;
  close_dist_min_height_ = config.close_dist_min_height ;

  ROS_INFO("Parameters have been reconfigured");
}
