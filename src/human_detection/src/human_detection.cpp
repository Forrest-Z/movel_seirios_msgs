...
HumanDetection::HumanDetection()
  : map_pc(new pcl::PointCloud<pcl::PointXYZ>)
...
void HumanDetection::poseCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
  ...
  if (crop_marker_pub_.getNumSubscribers() > 0)
  {
    ...
    points[0].x = msg->position.x + sqrt(pow(detection_range_, 2) * 2) * cos(M_PI / 4 + y);
    points[0].y = msg->position.y + sqrt(pow(detection_range_, 2) * 2) * sin(M_PI / 4 + y);
    points[0].z = 0;
    points[1].x = msg->position.x - sqrt(pow(detection_range_, 2) * 2) * sin(M_PI / 4 + y);
    points[1].y = msg->position.y + sqrt(pow(detection_range_, 2) * 2) * cos(M_PI / 4 + y);
    points[1].z = 0;
    points[2].x = msg->position.x - sqrt(pow(detection_range_, 2) * 2) * cos(M_PI / 4 + y);
    points[2].y = msg->position.y - sqrt(pow(detection_range_, 2) * 2) * sin(M_PI / 4 + y);
    points[2].z = 0;
    points[3].x = msg->position.x + sqrt(pow(detection_range_, 2) * 2) * sin(M_PI / 4 + y);
    points[3].y = msg->position.y - sqrt(pow(detection_range_, 2) * 2) * cos(M_PI / 4 + y);
    points[3].z = 0;
    points[4].x = msg->position.x + sqrt(pow(detection_range_, 2) * 2) * cos(M_PI / 4 + y);
    points[4].y = msg->position.y + sqrt(pow(detection_range_, 2) * 2) * sin(M_PI / 4 + y);
    points[4].z = 0;
...
void HumanDetection::cropCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in,
...
void HumanDetection::adaptiveClustering(
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in, pcl::IndicesPtr pc_indices,
...
void HumanDetection::checkDimensions(
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_in,
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_out)
{
  for (size_t j = 0; j < clusters_in.size(); j++)
  {
    ...
    if ((maxPoint.x - minPoint.x) < max_width_ && (maxPoint.y - minPoint.y) < max_width_ &&
        (maxPoint.x - minPoint.x) > min_width_ && (maxPoint.y - minPoint.y) > min_width_ &&
        fabs((maxPoint.x - minPoint.x) - (maxPoint.y - minPoint.y)) < max_width_length_diff_ &&
        (maxPoint.x - minPoint.x) + (maxPoint.y - minPoint.y) < min_width_ + max_width_)
    {
      ...
      {
        double distance =
            sqrt(pow(pcaCentroid[0] - robot_coordinate.x, 2) + pow(pcaCentroid[1] - robot_coordinate.y, 2));
...
void HumanDetection::filterStaticObjects(
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_in,
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_out)
{
  ...
  bool pass;
  {
    for (size_t i = 0; i < clusters_in.size(); i++)
    {
      ...
      {
        //! Filter out point cloud clusters with map points representing static obstacles within the range of bounding
        //! boxes
        ...
        else
        {
          ...
          if (d1 < map_inflation_dist_ || d2 < map_inflation_dist_ || d3 < map_inflation_dist_ ||
              d4 < map_inflation_dist_)
          {
            pass = false;
            break;
          }
...
void HumanDetection::filterLaser(
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_in,
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZI>::Ptr> >&
        clusters_out)
...
void HumanDetection::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in)
{
  ...
  {
    ROS_INFO_STREAM("[human_detection] fps = " << float(frames) / (float(clock() - start_time) / CLOCKS_PER_SEC)
