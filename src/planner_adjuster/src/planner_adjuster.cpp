...
bool PlannerAdjuster::setupTopics()
{
  ...
  goal_sub_ = nh_.subscribe("/pid_goal", 1,
                                           &PlannerAdjuster::goalCb, this);
...
void PlannerAdjuster::odometryCb(nav_msgs::Odometry msg)
{
  ...
  if (has_goal_)
  {
    geometry_msgs::TransformStamped transform = tf_buffer_.lookupTransform(current_goal_.header.frame_id, msg.header.frame_id, ros::Time(0));
...
void PlannerAdjuster::doControl(geometry_msgs::Pose current_pose)
{
  ...
  else if (controller_stage_ == 1)
  {
    ...
    u_th = angle_PID_init.update(theta, dt);
    // ROS_INFO("stage %d, goal %5.2f, state %5.2f, angle goal %5.2f, angle %5.2f", controller_stage_, dist_PID.getRef(), dx, angle_PID.getRef(), theta);
  }
  else if (controller_stage_ == 2)
  {
