...
static const char* keys = { "{@image_path1 | | Image path 1 }"
...
vector<DMatch> symmetric_test(std::vector<DMatch> good_matches1, std::vector<DMatch> good_matches2)
{
  ...
  for (int i = 0; i < good_matches1.size(); i++)
  {
    for (int j = 0; j < good_matches2.size(); j++)
    {
      ...
      {
        better_matches.push_back(
            DMatch(good_matches1[i].queryIdx, good_matches1[i].trainIdx, good_matches1[i].distance));
        break;
      }
    }
  }
