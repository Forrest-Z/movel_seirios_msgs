...
{
    LIOF::LIOF(int radius_threshold, int final_radius, float low_angle, float high_angle, int length_threshold, float distance_threshold, double canny_th1, double canny_th2, int canny_aperture_size, bool do_merge)
    {
    ...
    void LIOF::intersect(const Mat& mask, const vector<Vec4i>& lines, vector<KeyPoint>& keypoints)
    {
        for (int i = 0; i < lines.size(); i++)
        {
            ...
            for (int j = 0; j < lines.size() && i!=j; j++)
            {
                ...
                if (p.x > (mask.cols - radius_threshold_extended_) || p.x < radius_threshold_extended_ || p.y > (mask.rows - radius_threshold_extended_) || p.y < radius_threshold_extended_)
                    continue;
    ...
    void LIOF::toScharr(const Mat& image, Mat& output)
    {
        ...
        for (int i = 0; i < image.rows; i++)
        {
            ...
            {
                float magnitude = (sqrt(pow(dx.at<float>(i,j),2) + pow(dy.at<float>(i,j),2)) / 4800); //normalising to [0..1] range
    ...
    void LIOF::compute(const Mat& image,const vector<KeyPoint>& keypoints, Mat& descriptors)
    {
        ...
        for (int i = 0; i < keypoints.size(); i++)
        {
            ...
            Mat rot = getRotationMatrix2D(Point2f(radius_threshold_extended_, radius_threshold_extended_), 360 * keypoints[i].angle / (2 * M_PI),1);
            Mat rotated;
