...
#include "obstacle_pebble_planner/obstacle_pebble_planner.h"
#include <base_local_planner/footprint_helper.h>
PLUGINLIB_EXPORT_CLASS(obstacle_pebble_planner::PebbleLocalPlanner, nav_core::BaseLocalPlanner)
...
namespace obstacle_pebble_planner
{
  ...
  double  PebbleLocalPlanner::compute_max_line_cost(
								...
								const tf2::Vector3& world_pos_1)
  {
    auto cost_map = costmap_ptr_->getCostmap();
  ...
  bool PebbleLocalPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
  {
    ...
    if(enable_obstacle_check){
      ...
      // tf2::fromMsg(robot_pose.pose.position.x, actual_pose);
      bool have_obstacle = false;
      float obstacle_dist = 0.0;
      ...
      for (int i = idx_plan_; i < decimated_global_plan_.size()-2; i++)
      {
        if(obstacle_dist < 5) {  // 5 denotes the global plan range to check
          ...
          last_pose = pose;
          pose.setOrigin(tf2::Vector3(decimated_global_plan_[i].pose.position.x, decimated_global_plan_[i].pose.position.y, decimated_global_plan_[i].pose.position.z));
          pose.setRotation(tf2::Quaternion(decimated_global_plan_[i].pose.orientation.x, decimated_global_plan_[i].pose.orientation.y, decimated_global_plan_[i].pose.orientation.z, decimated_global_plan_[i].pose.orientation.w));
          // tf2::fromMsg(decimated_global_plan_[i].pose, pose);
          obstacle_dist += d_min_;
        }
      }
  ...
  void PebbleLocalPlanner::dynConfigCb(obstacle_pebble_planner::obstacle_pebble_plannerConfig &config, uint32_t level)
  {
