...
std::vector<double> FeatureDescription::calculateHistogram(const std::vector<double>& data_x,
                                                           const std::vector<double>& data_y,
                                                           const boost::array<double, 2>& point)
{
...
std::vector<double> FeatureDescription::calculateLineHistogram(const std::vector<double>& data_x,
                                                               const std::vector<double>& data_y, Line& line)
{
...
void FeatureDescription::setPointsHistogram(const std::vector<double>& data_x, const std::vector<double>& data_y,
...
void FeatureDescription::setLinesHistogram(const std::vector<double>& data_x, const std::vector<double>& data_y,
...
void FeatureDescription::match(std::vector<Line>& scan_lines, std::vector<Line>& map_lines)
{
  ...
  for (int k = 0; k < map_lines.size(); ++k)
  {
    ...
    double dist_match = sqrt(pow(scan_lines[match].getEnd()[1] - scan_lines[match].getStart()[1], 2) +
                             pow(scan_lines[match].getEnd()[0] - scan_lines[match].getStart()[0], 2));
    double dist_main = sqrt(pow(map_lines[k].getEnd()[1] - map_lines[k].getStart()[1], 2) +
                            pow(map_lines[k].getEnd()[0] - map_lines[k].getStart()[0], 2));
...
double FeatureDescription::matchScore(nav_msgs::OccupancyGrid::ConstPtr map, std::vector<double> scan_x,
