...
//    data_association.reset(new kkl::alg::GlobalNearestNeighborAssociation<KalmanTracker::Ptr, VisualDetection>());
   ...
   */
  void predict(const ros::Time& time, float linear, float ang) {
    for(auto& person : people) {
      person->predict(time, linear, ang);
    }
  }
  ...
  void correct(const ros::Time& time, const std::vector<Cluster>& detections, int target_id) {
    ...
    for(int i=0; i<detections.size(); i++) {
      if(!associated[i]) {
        ...
        for(const auto& person : people) {
          ...
          if((person->position() - observation_pos).norm() < human_radius_ * 2.0) {
    ...
    auto remove_loc = std::partition(people.begin(), people.end(), [&](const KalmanTracker::Ptr& tracker) {
      return (((ros::Time::now() - tracker->lastCorrectionTime()) < ros::Duration(remove_time_thresh_)) || (tracker->id() == long(target_id)));
...
public:
  ...
  double human_radius_;          // new tracks must be far from existing tracks than this value
  //double remove_trace_thresh_;   // tracks with larger covariance trace than this will be removed
