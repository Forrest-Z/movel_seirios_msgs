...
public:
  ...
  void predict(const ros::Time& time, float linear, float ang) {
    ...
    kalman_filter->controlMatrix(0, 0) = difftime;
    kalman_filter->controlMatrix(1, 1) = difftime;
    Eigen::Matrix<double, 2, 1> u = Eigen::Matrix<double, 2, 1>::Zero();
    u(0, 0) = -double(linear*cos(ang));
    u(1, 0) = -double(linear*sin(ang));
    
    kalman_filter->predict(u, linear, ang, difftime);
    last_prediction_time = time;
...
public:
  ...
  Eigen::Vector3d observed_position() const {
    return observed_pos;
  }
