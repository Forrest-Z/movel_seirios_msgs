...
------------ | ------------- | -------------
~markers | visualization_msgs::MarkerArray | Publishes to Rviz for visualization
~tracks | hdl_people_tracking::TrackArray | Publishes Tracks, of which the selected track will be used to calculate cmd_vel
/hdl_people_detection_nodelet/clusters | hdl_people_tracking::ClusterArray | Detections fed to our tracker


## Subscribed Topics
Topic Name | Message Type | Purpose
------------ | ------------- | -------------
/lslidar_c16/lslidar_point_cloud | sensor_msgs::PointCloud2 | PointCloud by 3d lidar, used for detections
/hdl_people_detection_nodelet/clusters | hdl_people_tracking::ClusterArray | Detections will be associated with tracks
/cmd_vel | geometry_msgs::Twist | To incorporate robot's own motion into Kalman Filter

## Services

Service Name | Input Type and Name | Output Type and Name | Purpose
------------ | ------------- | ------------- | -------------
/hdl_people_tracking/choose_target | int target_id | bool success | Follows selected target. Returns true and overwrites previous target (if applicable) if succeeded. Returns false if selected target not in TrackArray.
/hdl_people_tracking/clear_target | bool clear_target | bool target_cleared_success | Stop following the target person and cancel the last published goal to the target person, if the goal has not been reached.
/hdl_people_tracking/reset | bool restart | bool restarted | Reinitialize the tracker. Also calls hdl_people_tracking/clear_target. 
