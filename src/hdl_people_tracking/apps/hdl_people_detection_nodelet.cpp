...
public:
  ...
  void onInit() override {
    ...
    if(private_nh.param<bool>("static_sensor", false)) {
      static_points_sub = mt_nh.subscribe(pointcloud2_topic_, 32, &HdlPeopleDetectionNodelet::callback_static, this);
    } else {
      odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(mt_nh, "/odom", 20));
      points_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, pointcloud2_topic_, 20));
...
private:
  ...
  void initialize_params() {
    ...
    pointcloud2_topic_ = private_nh.param<std::string>("pointcloud2_topic", "/lslidar_c16/lslidar_point_cloud");
