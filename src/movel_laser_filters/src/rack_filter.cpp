#include "movel_laser_filters/rack_filter.h"
#include <ros/ros.h>
#include <movel_hasp_vendor/license.h>
#include <movel_laser_filters/RackLeg.h>

movel_laser_filters::RackFilter::RackFilter() : filter_enabled_(false),
                                          nh_private_("~")
{
  input_scan_sub_ = nh_private_.subscribe("scan_in", 1, &movel_laser_filters::RackFilter::onInputScan, this);
  filtered_scan_pub_ = nh_private_.advertise<sensor_msgs::LaserScan>("scan_out", 1);
  update_param_srv_ = nh_.advertiseService("/update_rack_filter", &movel_laser_filters::RackFilter::onUpdateParam, this);

  ros::spin();
}

bool movel_laser_filters::RackFilter::onUpdateParam(movel_laser_filters::RackFilterUpdateRequest &req, movel_laser_filters::RackFilterUpdateResponse &res)
{
  ...
  else
  {
    ...
    for (movel_laser_filters::RackLeg &leg : req.legs)
    {
      tf::Point min;
      min.setX(leg.min_x);
      min.setY(leg.min_y); 
...
void movel_laser_filters::RackFilter::onInputScan(const sensor_msgs::LaserScan &input_scan)
{
...
bool movel_laser_filters::RackFilter::inBox(tf::Point &point, Box& box)
{
  tf::Point min = box[0];
  tf::Point max = box[1];
...
int main(int argc, char **argv)
{
  ...
  movel_laser_filters::RackFilter node;
  #ifdef MOVEL_LICENSE
  ml.logout();
  #endif
  return 0;
