...
struct GlobalLocalizationResults
{
  ...
  {
    auto remove_loc =
        std::remove_if(results.begin(), results.end(), [](const auto& result) { return result == nullptr; });
    ...
    std::sort(results.begin(), results.end(),
              [](const auto& lhs, const auto& rhs) { return lhs->inlier_fraction > rhs->inlier_fraction; });
    if (results.size() > static_cast<long unsigned int>(max_num_candidates))
    {
