...
public:
  ...
  virtual double calc_matching_error(const pcl::PointCloud<pcl::PointXYZ>& cloud, const Eigen::Matrix4f& transformation,
                                     double* inlier_fraction) override
  {
    int num_inliers = 0;
    ...
    for (long unsigned int i = 0; i < transformed.size(); i++)
    {
