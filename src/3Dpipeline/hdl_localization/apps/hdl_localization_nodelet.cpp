...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    try
    {
      tf_listener.lookupTransform(odom_child_frame_id, points_msg->header.frame_id,  ros::Time(0), transform);
    ...
    if(private_nh.param<bool>("enable_robot_odometry_prediction", false) && !last_correction_time.isZero()) {
      tf::StampedTransform transform;
      if(tf_listener.waitForTransform(cloud->header.frame_id, ros::Time(0), cloud->header.frame_id, last_correction_time, robot_odom_frame_id, ros::Duration(0))) {
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    tf::StampedTransform transform_inv;
    tf_listener.waitForTransform(odom_child_frame_id, "base_link", ros::Time(0), ros::Duration(3));
    tf_listener.lookupTransform("base_link", odom_child_frame_id,  ros::Time(0), transform_inv);

    tf::Transform transform_final;
    transform_final = transform * transform_inv;
    tf::Vector3 vf = transform_final.getOrigin();
    tf::Quaternion qf = transform_final.getRotation();
    tf::Vector3 axis= qf.getAxis();
    double ang = qf.getAngle();
    double x = axis.x() * sin(ang/2);
    double y = axis.y() * sin(ang/2);
    double z = axis.z() * sin(ang/2);
    double w = cos(ang/2);
    Eigen::Vector3f pos(vf.x(), vf.y(), vf.z());
...
/*
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    ROS_INFO("Quat: %f %f %f %f %f", q.x, q.y, q.z, q.w);
    pose_estimator.reset(
          new hdl_localization::PoseEstimator(
            registration,
            ros::Time::now(),
...
private:
  ...
  //std::atomic<std::bool> relocalizing;
