...
#include <iostream>
// For disable PCL complile lib, to use PointXYZIRT
...
private:
  ...
  int num_iter_;
  int num_lpr_;
  double th_seeds_upper, th_seeds_lower;
  double th_dist_;
...
GroundPlaneFit::GroundPlaneFit() : node_handle_("~")
{
  ...
  node_handle_.param<std::string>("point_topic", point_topic_, "/cloud_pcd");
  ...
  node_handle_.param("th_seeds_upper", th_seeds_upper, 1.2);  // from center to below
  ROS_INFO("Seeds Threshold Upper: %f", th_seeds_upper);

  node_handle_.param("th_seeds_lower", th_seeds_lower, 1.0);
  ROS_INFO("Seeds Threshold Lower: %f", th_seeds_lower);

  node_handle_.param("th_dist", th_dist_, 0.3);
  ROS_INFO("Distance Threshold: %f", th_dist_);

  node_handle_.param("sor_mean_k", sor_mean_k_, 50);
  ROS_INFO("SOR mean K: %d", sor_mean_k_);

  node_handle_.param("sor_sigma_mult", sor_sigma_mult_, 1.0);
  ROS_INFO("SOR sigma multiplier %5.2f", sor_sigma_mult_);
...
/*
    ...
    @param ::th_seeds_upper: threshold distance of seeds
    @param ::
...
void GroundPlaneFit::extract_initial_seeds_(const pcl::PointCloud<VPoint>& p_sorted)
{
  ...
  double lpr_height = cnt != 0 ? sum / cnt : 0;  // in case divide by 0 //(average of sum)
  g_seeds_pc->clear();
  // iterate pointcloud, filter those height is less than lpr.height+th_seeds_upper
  for (int i = 0; i < p_sorted.points.size(); i++)
  {
    if (p_sorted.points[i].z < lpr_height + th_seeds_upper)
    {
      if (p_sorted.points[i].z > lpr_height + th_seeds_upper - th_seeds_lower)
      {
...
void GroundPlaneFit::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg)
{
  ...
  for (int i = 0; i < laserCloudIn.points.size(); i++)
  {
    ...
    else if (laserCloudIn.points[i].z < 0.0)
    {
      it++;
    }
    else
  ...
  pcl::PointCloud<VPoint>::iterator eit = laserCloudIn.points.end();
  for (int i = laserCloudIn.points.size(); i >= 0; i--)
  {
    if (laserCloudIn.points[i].z > 2.0)
    {
      eit--;
    }
  }
  laserCloudIn.points.erase(eit, laserCloudIn.points.end());
  ...
  pcl::PointCloud<VPoint>::Ptr not_ground_filtered(new pcl::PointCloud<VPoint>);
  pcl::StatisticalOutlierRemoval<VPoint> sor;
  sor.setInputCloud(g_not_ground_pc);
  sor.setMeanK(sor_mean_k_);
  sor.setStddevMulThresh(sor_sigma_mult_);
  sor.filter(*not_ground_filtered);
  ...
  pcl::toROSMsg(*not_ground_filtered, groundless_msg);
