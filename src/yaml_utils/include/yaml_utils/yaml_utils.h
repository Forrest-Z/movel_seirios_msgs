...
YAML::Node parseArray(XmlRpc::XmlRpcValue &xml, std::string &name)
{
  ...
  for (int i = 0; i < xml.size(); ++i)
  {
    ...
    {
      case 0: ROS_FATAL("%s", "XML of TypeInvalid detected. Exiting");
              break;
      case 1: element = parseBool(xml[i], name);
              break;
      case 2: element = parseInt(xml[i], name);
              break;
      case 3: element = parseDouble(xml[i], name);
              break;
      case 4: element = parseString(xml[i], name);
              break;
      case 5: element = parseDateTime(xml[i], name);
              break;
      case 6: element = parseBase64(xml[i], name);
              break;
      case 7: element = parseArray(xml[i], name);
              break;
      case 8: element = parseStruct(xml[i], name);
              break;
      default: ROS_FATAL("%s", "Unrecognized XML type detected. Exiting");
               break;
    }
...
YAML::Node parseStruct(XmlRpc::XmlRpcValue &xml, std::string &name)
{
  ...
  for(iter = xml.begin(); iter != xml.end(); ++iter)
  {
    ...
    switch (value.getType())
    {
      case 0: ROS_FATAL("%s", "XML of TypeInvalid detected. Exiting");
              break;
      case 1: branch = parseBool(value, key);
              break;
      case 2: branch = parseInt(value, key);
              break;
      case 3: branch = parseDouble(value, key);
              break;
      case 4: branch = parseString(value, key);
              break;
      case 5: branch = parseDateTime(value, key);
              break;
      case 6: branch = parseBase64(value, key);
              break;
      case 7: branch = parseArray(value, key);
              break;
      case 8: branch = parseStruct(value, key);
              break;
      default: ROS_FATAL("%s", "Unrecognized XML type detected. Exiting");
               break;
    }
...
namespace YamlUtils
{
  YAML::Node xmlToYaml(XmlRpc::XmlRpcValue &xml, std::string &name)
  {
    ...
    switch (xml.getType())
    {
      case 0: ROS_FATAL("%s", "XML of TypeInvalid detected. Exiting");
              break;
      case 1: yaml[name] = parseBool(xml, name);
              break;
      case 2: yaml[name] = parseInt(xml, name);
              break;
      case 3: yaml[name] = parseDouble(xml, name);
              break;
      case 4: yaml[name] = parseString(xml, name);
              break;
      case 5: yaml[name] = parseDateTime(xml, name);
              break;
      case 6: yaml[name] = parseBase64(xml, name);
              break;
      case 7: yaml[name] = parseArray(xml, name);
              break;
      case 8: yaml[name] = parseStruct(xml, name);
              break;
      default: ROS_FATAL("%s", "Unrecognized XML type detected. Exiting");
               break;
    }
